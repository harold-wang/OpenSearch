...
package org.renameme.benchmark.search.aggregations.bucket.terms;

import org.apache.lucene.util.BytesRef;
import org.renameme.common.io.stream.DelayableWriteable;
...
public class StringTermsSerializationBenchmark {
    private static final NamedWriteableRegistry REGISTRY = new NamedWriteableRegistry(
        org.renameme.common.collect.List.of(new NamedWriteableRegistry.Entry(InternalAggregation.class, StringTerms.NAME, StringTerms::new))
    );
    @Param(value = { "1000" })
    private int buckets;
    ...
    public void initResults() {
        results = DelayableWriteable.referencing(InternalAggregations.from(org.renameme.common.collect.List.of(newTerms(true))));
    }
    ...
    private StringTerms newTerms(boolean withNested) {
        ...
        for (int i = 0; i < buckets; i++) {
            InternalAggregations inner = withNested
                ? InternalAggregations.from(org.renameme.common.collect.List.of(newTerms(false)))
