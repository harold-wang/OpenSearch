...
public abstract class AbstractBroadcastResponseTestCase<T extends BroadcastResponse> extends AbstractXContentTestCase<T> {
    ...
    protected T createTestInstance() {
        ...
        if (failedShards > 0) {
            ...
            for (int i = 0; i < failedShards; i++) {
                RenamemeException exception = new RenamemeException("exception message " + i);
    ...
    public void testFailuresDeduplication() throws IOException {
        ...
        RenamemeException exception1 = new RenamemeException("foo", new IllegalArgumentException("bar"));
        exception1.setIndex(index);
        exception1.setShard(new ShardId(index, 0));
        RenamemeException exception2 = new RenamemeException("foo", new IllegalArgumentException("bar"));
        exception2.setIndex(index);
        exception2.setShard(new ShardId(index, 1));
        RenamemeException exception3 = new RenamemeException("fizz", new IllegalStateException("buzz"));
