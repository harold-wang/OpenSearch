...
package org.renameme.index.alias;
...
public final class RandomAliasActionsGenerator {
    ...
    public static AliasActions randomAliasAction(boolean useStringAsFilter) {
        AliasActions action = new AliasActions(ESTestCase.randomFrom(AliasActions.Type.values()));
        ...
        if (action.actionType() == AliasActions.Type.ADD) {
            if (ESTestCase.randomBoolean()) {
                if (useStringAsFilter) {
                    ...
                    action.filter(randomMap(ESTestCase.randomInt(5)));
                }
            }
            if (ESTestCase.randomBoolean()) {
                if (ESTestCase.randomBoolean()) {
            ...
            if (ESTestCase.randomBoolean()) {
                action.writeIndex(ESTestCase.randomBoolean());
            }
            if (ESTestCase.randomBoolean()) {
                action.isHidden(ESTestCase.randomBoolean());
            }
        }
        return action;
    }
    ...
    public static Object randomRouting() {
        return ESTestCase.randomBoolean() ? ESTestCase.randomAlphaOfLength(5) : ESTestCase.randomInt();
    }
}
