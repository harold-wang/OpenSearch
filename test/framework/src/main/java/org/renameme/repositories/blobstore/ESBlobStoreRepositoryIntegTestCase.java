...
import org.renameme.snapshots.SnapshotRestoreException;
...
public abstract class ESBlobStoreRepositoryIntegTestCase extends ESIntegTestCase {
    ...
    protected final String createRepository(final String name, final Settings settings) {
        ...
        RenamemeAssertions.assertAcked(client().admin().cluster().preparePutRepository(name)
    ...
    public void testSnapshotAndRestore() throws Exception {
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            RenamemeAssertions.assertHitCount(client().prepareSearch(indexNames[i]).setSize(0).get(), docCounts[i]);
        }
        ...
        if (deleteIndices.size() > 0) {
            ...
            RenamemeAssertions.assertAcked(client().admin().indices().prepareDelete(deleteIndices.toArray(new String[deleteIndices.size()])));
        }
        ...
        if (closeIndices.size() > 0) {
            ...
            RenamemeAssertions.assertAcked(client().admin().indices().prepareClose(closeIndices.toArray(new String[closeIndices.size()])));
        }
        ...
        RenamemeAssertions.assertAcked(client().admin().cluster().prepareDeleteSnapshot(repoName, snapshotName).get());
    ...
    public void testMultipleSnapshotAndRollback() throws Exception {
        ...
        RenamemeAssertions.assertAcked(client().admin().indices().prepareCreate(indexName).get());
        ...
        for (int i = 0; i < restoreOperations; i++) {
            ...
            RenamemeAssertions.assertAcked(client().admin().indices().prepareClose(indexName));
            ...
            ensureGreen();
            RenamemeAssertions.assertHitCount(client().prepareSearch(indexName).setSize(0).get(), docCounts[iterationToRestore]);
        }

        for (int i = 0; i < iterationCount; i++) {
            logger.info("-->  delete snapshot {}:{}", repoName, snapshotName + "-" + i);
            RenamemeAssertions.assertAcked(client().admin().cluster().prepareDeleteSnapshot(repoName, snapshotName + "-" + i).get());
        }
    }
    ...
    public void testIndicesDeletedFromRepository() throws Exception {
        ...
        RenamemeAssertions.assertAcked(client().admin().cluster().prepareDeleteSnapshot(repoName, "test-snap").get());
        ...
        RenamemeAssertions.assertAcked(client().admin().cluster().prepareDeleteSnapshot(repoName, "test-snap2").get());
    }
