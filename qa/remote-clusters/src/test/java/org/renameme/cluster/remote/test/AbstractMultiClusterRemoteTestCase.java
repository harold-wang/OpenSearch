 ...
 * under the License.
 */
package org.renameme.cluster.remote.test;
...
public abstract class AbstractMultiClusterRemoteTestCase extends ESRestTestCase {
    ...
    protected String getTestRestCluster() {
        return "localhost:" + getProperty("test.fixtures.renameme-" + getDistribution() + "-1.tcp.9200");
    }
    ...
    public void initClientsAndConfigureClusters() throws Exception {
        ...
        cluster1Client = buildClient("localhost:" + getProperty("test.fixtures.renameme-" + getDistribution() + "-1.tcp.9200"));
        cluster2Client = buildClient("localhost:" + getProperty("test.fixtures.renameme-" + getDistribution() + "-2.tcp.9200"));
    ...
    public static void getKeyStore() {
        try {
            ...
            throw new RenamemeException("exception while reading the store", e);
        }
    ...
    private String getProperty(String key) {
        ...
        if (value == null) {
            throw new IllegalStateException("Could not find system properties from test.fixtures. " +
                "This test expects to run with the renameme.test.fixtures Gradle plugin");
        }
        return value;
    }
}
