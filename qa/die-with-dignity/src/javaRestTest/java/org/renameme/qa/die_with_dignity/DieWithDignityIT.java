...
package org.renameme.qa.die_with_dignity;
...
public class DieWithDignityIT extends ESRestTestCase {
    public void testDieWithDignity() throws Exception {
        ...
        // the Renameme process should die and disappear from the output of jps
        assertBusy(() -> {
        ...
        // parse the logs and ensure that Renameme died with the expected cause
        ...
        try {
            while (it.hasNext() && (fatalError == false || fatalErrorInThreadExiting == false)) {
                ...
                if (line.matches(".*ERROR.*o\\.e\\.ExceptionsHelper.*javaRestTest-0.*fatal error.*")) {
                    fatalError = true;
                } else if (line.matches(".*ERROR.*o\\.e\\.b\\.RenamemeUncaughtExceptionHandler.*javaRestTest-0.*"
