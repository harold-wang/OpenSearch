...
public class PluginCliTests extends PackagingTestCase {
    ...
    public void test20SymlinkPluginsDir() throws Exception {
        ...
        Platforms.onLinux(() -> sh.run("chown renameme:renameme " + linkedPlugins.toString()));
        ...
        assertWithExamplePlugin(installResult -> {
            assertWhileRunning(() -> {
                ...
                final String settingsResponse = ServerUtils.makeRequest(Request.Get("http://localhost:9200/" + settingsPath)).trim();
    ...
    public void test23RenamemeWithSpace() throws Exception {
        assumeTrue(distribution.isArchive());

        Path spacedDir = createTempDir("spaced dir");
        Path renameme = spacedDir.resolve("renameme");
        Files.move(installation.home, renameme);
        Installation spacedInstallation = Installation.ofArchive(sh, distribution, renameme);

        assertWithPlugin(spacedInstallation.executables().pluginTool, EXAMPLE_PLUGIN_ZIP, EXAMPLE_PLUGIN_NAME, installResult -> {});

        Files.move(renameme, installation.home);
    }
