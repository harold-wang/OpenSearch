...
/**
 * Represents an installation of Renameme
 */
public class Installation {
    ...
    public static Installation ofPackage(Shell sh, Distribution distribution) {
        ...
        final Path envFile = (distribution.packaging == Distribution.Packaging.RPM)
            ? Paths.get("/etc/sysconfig/renameme")
            : Paths.get("/etc/default/renameme");

        return new Installation(
            sh,
            distribution,
            Paths.get("/usr/share/renameme"),
            Paths.get("/etc/renameme"),
            Paths.get("/var/lib/renameme"),
            Paths.get("/var/log/renameme"),
            Paths.get("/usr/share/renameme/plugins"),
            Paths.get("/usr/share/renameme/modules"),
            Paths.get("/var/run/renameme"),
            envFile
        );
    }

    public static Installation ofContainer(Shell sh, Distribution distribution) {
        String root = "/usr/share/renameme";
        return new Installation(
            sh,
            distribution,
            Paths.get(root),
    ...
    public class Executables {
        ...
        public final Executable renameme = new Executable("renameme");
        public final Executable pluginTool = new Executable("renameme-plugin");
        public final Executable keystoreTool = new Executable("renameme-keystore");
        public final Executable shardTool = new Executable("renameme-shard");
        public final Executable nodeTool = new Executable("renameme-node");
    }
}
