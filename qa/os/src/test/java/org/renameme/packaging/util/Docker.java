...
import static org.renameme.packaging.util.FileMatcher.p644;
import static org.renameme.packaging.util.FileMatcher.p660;
import static org.renameme.packaging.util.FileMatcher.p755;
import static org.renameme.packaging.util.FileMatcher.p770;
import static org.renameme.packaging.util.FileMatcher.p775;
...
/**
 * Utilities for running packaging tests against the Renameme Docker images.
 */
public class Docker {
    ...
    /**
     * Runs an Renameme Docker container.
    ...
    /**
     * Runs an Renameme Docker container, with options for overriding the config directory
    ...
    public static Installation runContainer(
        ...
        waitForRenamemeToStart();

        return Installation.ofContainer(dockerShell, distribution);
    }

    /**
     * Similar to {@link #runContainer(Distribution, Map, Map)} in that it runs an Renameme Docker
    ...
    private static void executeDockerRun(
        ...
        if (volumes != null) {
            volumes.forEach((localPath, containerPath) -> {
                MatcherAssert.assertThat(localPath, FileExistenceMatchers.fileExists());

                if (Platforms.WINDOWS == false && System.getProperty("user.name").equals("root") && uid == null) {
                    // The tests are running as root, but the process in the Docker container runs as `renameme` (UID 1000),
    ...
    /**
     * Waits for the Renameme process to start executing in the container.
     * This is called every time a container is started.
     */
    public static void waitForRenamemeToStart() {
        boolean isRenamemeRunning = false;
        int attempt = 0;
        ...
        do {
            try {
                ...
                if (psOutput.contains("org.renameme.bootstrap.Renameme")) {
                    isRenamemeRunning = true;
                    break;
                }
            } catch (Exception e) {
        ...
        if (isRenamemeRunning == false) {
            final Shell.Result dockerLogs = getContainerLogs();
            fail(
                "Renameme container did not start successfully.\n\nps output:\n"
                    + psOutput
                    + "\n\nStdout:\n"
                    + dockerLogs.stdout
                    + "\n\nStderr:\n"
    ...
    /**
     * Waits for the Renameme container to exit.
     */
    private static void waitForRenamemeToExit() {
        boolean isRenamemeRunning = true;
        int attempt = 0;
        ...
        if (isRenamemeRunning) {
            final Shell.Result dockerLogs = getContainerLogs();
            fail("Renameme container did exit.\n\nStdout:\n" + dockerLogs.stdout + "\n\nStderr:\n" + dockerLogs.stderr);
        }
    }
    ...
    public static class DockerShell extends Shell {
        ...
        protected String[] getScriptCommand(String script) {
            ...
            return super.getScriptCommand("docker exec --user renameme:root --tty " + containerId + " " + script);
        }
    }
    ...
    public static void assertPermissionsAndOwnership(Path path, Set<PosixFilePermission> expectedPermissions) {
        ...
        assertThat("File owner of " + path + " is wrong", username, equalTo("renameme"));
    ...
    private static void verifyOssInstallation(Installation es) {
        dockerShell.run("id renameme");
        dockerShell.run("getent group renameme");

        final Shell.Result passwdResult = dockerShell.run("getent passwd renameme");
        final String homeDir = passwdResult.stdout.trim().split(":")[5];
        assertThat(homeDir, equalTo("/usr/share/renameme"));
        ...
        assertThat(dockerShell.run(es.bin("renameme-keystore") + " list").stdout, containsString("keystore.seed"));
    ...
    public static void waitForRenameme(Installation installation) throws Exception {
        withLogging(() -> ServerUtils.waitForRenameme(installation));
    }

    public static void waitForRenameme(String status, String index, Installation installation, String username, String password)
        throws Exception {
        withLogging(() -> ServerUtils.waitForRenameme(status, index, installation, username, password));
    }
    ...
    private static <E extends Exception> void withLogging(CheckedRunnable<E> r) throws Exception {
        try {
            ...
            logger.warn("Renameme container failed to start.\n\nStdout:\n" + logs.stdout + "\n\nStderr:\n" + logs.stderr);
            throw e;
        }
    }
