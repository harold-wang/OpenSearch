...
import org.renameme.packaging.util.Archives;
...
public abstract class PackagingTestCase extends Assert {
    ...
    public static void init() throws Exception {
        ...
        if (distribution().isDocker()) {
            Docker.ensureImageIsLoaded(distribution);
    ...
    public void teardown() throws Exception {
        if (installation != null && failed == false) {
            if (Files.exists(installation.logs)) {
                Path logFile = installation.logs.resolve("renameme.log");
                if (Files.exists(logFile)) {
                    logger.warn("Renameme log:\n" + FileUtils.slurpAllLogs(installation.logs, "renameme.log", "*.log.gz"));
                }
                ...
                if (Files.exists(logFile)) {
                    Path newFile = installation.logs.resolve(prefix + ".renameme.log");
                    FileUtils.mv(logFile, newFile);
                }
                for (Path rotatedLogFile : FileUtils.lsGlob(installation.logs, "renameme*.tar.gz")) {
    ...
    protected static void cleanup() throws Exception {
        installation = null;
        Cleanup.cleanEverything();
    }

    /**
     * Starts and stops renameme, and performs assertions while it is running.
     */
    protected void assertWhileRunning(Platforms.PlatformAction assertions) throws Exception {
        try {
            awaitRenamemeStartup(runRenamemeStartCommand(null, true, false));
        } catch (Exception e) {
            ...
            if (Files.exists(installation.logs.resolve("renameme.log"))) {
                logger.warn("Renameme log:\n" + FileUtils.slurpAllLogs(installation.logs, "renameme.log", "*.log.gz"));
            }
    ...
    /**
     * Run the command to start Renameme, but don't wait or test for success.
     * This method is useful for testing failure conditions in startup. To await success,
     * use {@link #startRenameme()}.
     ...
     * @param daemonize Run Renameme in the background
     ...
     */
    public Shell.Result runRenamemeStartCommand(String password, boolean daemonize, boolean useTty) throws Exception {
        if (password != null) {
        ...
        switch (distribution.packaging) {
            ...
            case ZIP:
                if (useTty) {
                    return Archives.startRenamemeWithTty(installation, sh, password, daemonize);
                } else {
                    return Archives.runRenamemeStartCommand(installation, sh, password, daemonize);
                }
            case DEB:
            case RPM:
                return Packages.runRenamemeStartCommand(sh);
            case DOCKER:
            case DOCKER_UBI:
    ...
    public void stopRenameme() throws Exception {
        switch (distribution.packaging) {
            case TAR:
            case ZIP:
                Archives.stopRenameme(installation);
                break;
            case DEB:
            case RPM:
                Packages.stopRenameme(sh);
                break;
            case DOCKER:
            case DOCKER_UBI:
                // nothing, "installing" docker image is running it
    ...
    public void awaitRenamemeStartup(Shell.Result result) throws Exception {
        assertThat("Startup command should succeed", result.exitCode, equalTo(0));
        switch (distribution.packaging) {
            case TAR:
            case ZIP:
                Archives.assertRenamemeStarted(installation);
                break;
            case DEB:
            case RPM:
                Packages.assertRenamemeStarted(sh, installation);
                break;
            case DOCKER:
            case DOCKER_UBI:
                Docker.waitForRenamemeToStart();
                break;
            default:
                throw new IllegalStateException("Unknown Renameme packaging type.");
        }
    ...
    /**
     * Start Renameme and wait until it's up and running. If you just want to run
     * the start command, use {@link #runRenamemeStartCommand(String, boolean, boolean)}.
     * @throws Exception if Renameme can't start
     */
    public void startRenameme() throws Exception {
        awaitRenamemeStartup(runRenamemeStartCommand(null, true, false));
    }

    public void assertRenamemeFailure(Shell.Result result, String expectedMessage, Packages.JournaldWrapper journaldWrapper) {
        assertRenamemeFailure(result, Collections.singletonList(expectedMessage), journaldWrapper);
    }

    public void assertRenamemeFailure(Shell.Result result, List<String> expectedMessages, Packages.JournaldWrapper journaldWrapper) {
        ...
        if (Files.exists(installation.logs.resolve("renameme.log"))) {
            ...
            assertThat(installation.logs.resolve("renameme.log"), FileExistenceMatchers.fileExists());
            String logfile = FileUtils.slurp(installation.logs.resolve("renameme.log"));
            ...
            assertThat(result.stderr, containsString("Job for renameme.service failed"));
     ...
     * test to execute, and running Renameme with {@link #startRenameme()} will
    ...
    public void withCustomConfig(CheckedConsumer<Path, Exception> action) throws Exception {
        ...
        Path tempConf = tempDir.resolve("renameme");
        FileUtils.copyDirectory(installation.config, tempConf);

        Platforms.onLinux(() -> sh.run("chown -R renameme:renameme " + tempDir));

        if (distribution.isPackage()) {
            Files.copy(installation.envFile, tempDir.resolve("renameme.bk"));// backup
            FileUtils.append(installation.envFile, "ES_PATH_CONF=" + tempConf + "\n");
        } else {
        ...
        if (distribution.isPackage()) {
            ...
            Files.copy(tempDir.resolve("renameme.bk"), installation.envFile);
        } else {
