...
package org.renameme.index.reindex.remote;
...
import org.renameme.client.ResponseListener;
...
import org.renameme.index.reindex.RejectAwareActionListener;
...
import static org.renameme.common.unit.TimeValue.timeValueNanos;
import static org.renameme.index.reindex.remote.RemoteResponseParsers.MAIN_ACTION_PARSER;
import static org.renameme.index.reindex.remote.RemoteResponseParsers.RESPONSE_PARSER;
...
public class RemoteScrollableHitSource extends ScrollableHitSource {
    ...
    protected void clearScroll(String scrollId, Runnable onCompletion) {
        client.performRequestAsync(RemoteRequestBuilders.clearScroll(scrollId, remoteVersion), new ResponseListener() {
            @Override
            public void onSuccess(org.renameme.client.Response response) {
                ...
                if (e instanceof ResponseException) {
                    ResponseException re = (ResponseException) e;
                        ...
                        logger.debug((Supplier<?>) () -> new ParameterizedMessage(
                                "Failed to clear scroll [{}] from pre-2.0 Renameme. This is normal if the request terminated "
    ...
    protected void cleanup(Runnable onCompletion) {
         ...
         * asynchronously on a thread in Renameme's generic thread pool. */
                        ...
                        try {
                            ...
                            if (xContentType == null) {
                                try {
                                    ...
                                    RenamemeException ee = new RenamemeException("Error extracting body from response");
                                    ee.addSuppressed(e);
                                    throw ee;
                                }
                            }
                            ...
                            try (XContentParser xContentParser = xContentType.xContent().createParser(NamedXContentRegistry.EMPTY,
                                 ...
                                 * is totally wrong and we're probably not talking to Renameme. */
                                throw new RenamemeException(
                                    "Error parsing the response, remote is likely not an Renameme instance", e);
                            }
                        } catch (IOException e) {
                            throw new RenamemeException(
                                "Error deserializing response, remote is likely not an Renameme instance", e);
                        }
     ...
     * body in the message so the user can figure out *why* the remote Renameme service threw the error back to us.
     */
    static RenamemeStatusException wrapExceptionToPreserveStatus(int statusCode, @Nullable HttpEntity entity, Exception cause) {
        ...
        try {
            return new RenamemeStatusException(messagePrefix + bodyMessage(entity), status, cause);
        } catch (IOException ioe) {
            RenamemeStatusException e = new RenamemeStatusException(messagePrefix + "Failed to extract body.", status, cause);
            e.addSuppressed(ioe);
            return e;
        }
    }
