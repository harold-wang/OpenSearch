...
package org.renameme.script.expression;
...
import org.renameme.search.aggregations.pipeline.SimpleValue;
...
import static org.renameme.search.aggregations.PipelineAggregatorBuilders.bucketScript;
...
public class MoreExpressionIT extends ESIntegTestCase {
    ...
    public void testDateMethods() throws Exception {
        RenamemeAssertions.assertAcked(prepareCreate("test").addMapping("doc", "date0", "type=date", "date1", "type=date"));
        ensureGreen("test");
        indexRandom(true,
    ...
    public void testMultiValueMethods() throws Exception {
        RenamemeAssertions.assertAcked(prepareCreate("test").addMapping("doc",
                "double0", "type=double",
                "double1", "type=double",
                "double2", "type=double"));
        ensureGreen("test");
    ...
    public void testInvalidDateMethodCall() throws Exception {
        RenamemeAssertions.assertAcked(prepareCreate("test").addMapping("doc", "double", "type=double"));
        ensureGreen("test");
    ...
    public void testSparseField() throws Exception {
        RenamemeAssertions.assertAcked(prepareCreate("test").addMapping("doc", "x", "type=long", "y", "type=long"));
        ensureGreen("test");
        indexRandom(true,
        ...
        RenamemeAssertions.assertSearchResponse(rsp);
        SearchHits hits = rsp.getHits();
        assertEquals(2, rsp.getHits().getTotalHits().value);
        assertEquals(5.0, hits.getAt(0).field("foo").getValue(), 0.0D);
