...
package org.renameme.join.aggregations;
...
import org.renameme.search.aggregations.bucket.MultiBucketsAggregation;
...
import static org.renameme.join.aggregations.JoinAggregationBuilders.parent;
...
public class ParentIT extends AbstractParentChildTestCase {
    ...
    public void testSimpleParentAgg() throws Exception {
        final SearchRequestBuilder searchRequest = ESIntegTestCase.client().prepareSearch("test")
            .setSize(10000)
        ...
        Assert.assertThat("Request: " + searchRequest + "\nResponse: " + searchResponse + "\n",
        ...
        Assert.assertThat("Buckets: " + categoryTerms.getBuckets().stream().map(
        ...
        for (Map.Entry<String, Control> entry : categoryToControl.entrySet()) {
            ...
            if(entry.getValue().commentIds.isEmpty()) {
                Assert.assertNull(categoryTerms.getBucketByKey(entry.getKey()));
                continue;
            }

            final Terms.Bucket categoryBucket = categoryTerms.getBucketByKey(entry.getKey());
            Assert.assertNotNull("Failed for category " + entry.getKey(),
                categoryBucket);
            Assert.assertThat("Failed for category " + entry.getKey(),
    ...
    public void testParentAggs() throws Exception {
        ...
        for (Terms.Bucket commenterBucket : categoryTerms.getBuckets()) {
            ...
            Assert.assertThat("Failed for commenter " + commenterBucket.getKeyAsString(),
                commenterBucket.getDocCount(), equalTo((long)comments.size()));

            Parent articleAgg = commenterBucket.getAggregations().get("to_article");
            Assert.assertThat(articleAgg.getName(), equalTo("to_article"));
            ...
            Assert.assertThat(articleAgg.getDocCount(), equalTo((long)articles.size()));

            Terms categoryAgg = articleAgg.getAggregations().get("to_category");
            Assert.assertNotNull(categoryAgg);
            ...
            for (String category : categories) {
                ...
                Assert.assertNotNull(categoryBucket);

                Aggregation topCategory = categoryBucket.getAggregations().get("top_category");
                Assert.assertNotNull(topCategory);
            }
        }

        for (String commenter : commenters) {
            Terms.Bucket categoryBucket = categoryTerms.getBucketByKey(commenter);
            Assert.assertThat(categoryBucket.getKeyAsString(), equalTo(commenter));
            Assert.assertThat(categoryBucket.getDocCount(), equalTo((long) commenterToComments.get(commenter).size()));

            Parent childrenBucket = categoryBucket.getAggregations().get("to_article");
            Assert.assertThat(childrenBucket.getName(), equalTo("to_article"));
        }
    }
    ...
    public void testNonExistingParentType() throws Exception {
        SearchResponse searchResponse = ESIntegTestCase.client().prepareSearch("test")
            .addAggregation(
        ...
        Assert.assertThat(parent.getName(), equalTo("non-existing"));
        Assert.assertThat(parent.getDocCount(), equalTo(0L));
    }
