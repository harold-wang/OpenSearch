...
package org.renameme.painless.lookup;

import org.renameme.bootstrap.BootstrapInfo;
import org.renameme.painless.Def;
import org.renameme.painless.MethodWriter;
import org.renameme.painless.WriterConstants;
import org.renameme.painless.spi.Whitelist;
import org.renameme.painless.spi.WhitelistClass;
import org.renameme.painless.spi.WhitelistClassBinding;
import org.renameme.painless.spi.WhitelistConstructor;
import org.renameme.painless.spi.WhitelistField;
import org.renameme.painless.spi.WhitelistInstanceBinding;
import org.renameme.painless.spi.WhitelistMethod;
import org.renameme.painless.spi.annotation.InjectConstantAnnotation;
import org.renameme.painless.spi.annotation.NoImportAnnotation;
...
public final class PainlessLookupBuilder {
    ...
    public void addPainlessClass(Class<?> clazz, boolean importClassName) {
        ...
        if (clazz == def.class) {
            throw new IllegalArgumentException("cannot add reserved class [" + PainlessLookupUtility.DEF_CLASS_NAME + "]");
        }
        ...
        if (canonicalClassName.equals(importedCanonicalClassName)) {
            ...
            if (importedClass == null) {
                ...
                throw new IllegalArgumentException("imported class [" + importedCanonicalClassName + "] cannot represent multiple " +
                        "classes [" + canonicalClassName + "] and [" + PainlessLookupUtility.typeToCanonicalTypeName(importedClass) + "]");
    ...
    public void addPainlessConstructor(Class<?> targetClass, List<Class<?>> typeParameters, Map<Class<?>, Object> annotations) {
        ...
        if (targetClass == def.class) {
            throw new IllegalArgumentException("cannot add constructor to reserved class [" + PainlessLookupUtility.DEF_CLASS_NAME + "]");
        }
        ...
        if (painlessClassBuilder == null) {
            throw new IllegalArgumentException("target class [" + targetCanonicalClassName + "] not found" +
                    "for constructor [[" + targetCanonicalClassName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters)  + "]");
        }
        ...
        for (Class<?> typeParameter : typeParameters) {
            if (isValidType(typeParameter) == false) {
                throw new IllegalArgumentException("type parameter [" + PainlessLookupUtility.typeToCanonicalTypeName(typeParameter) + "] not found " +
                        "for constructor [[" + targetCanonicalClassName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]");
            }
        ...
        try {
            ...
            throw new IllegalArgumentException("reflection object not found for constructor " +
                    "[[" + targetCanonicalClassName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]", nsme);
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("method handle not found for constructor " +
                    "[[" + targetCanonicalClassName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]", iae);
        }
        ...
        if (existingPainlessConstructor == null) {
            ...
            throw new IllegalArgumentException("cannot add constructors with the same arity but are not equivalent for constructors " +
                    "[[" + targetCanonicalClassName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] and " +
                    "[[" + targetCanonicalClassName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(existingPainlessConstructor.typeParameters) + "]");
        }
    }
    ...
    public void addPainlessMethod(Class<?> targetClass, Class<?> augmentedClass,
        ...
        if (targetClass == def.class) {
            throw new IllegalArgumentException("cannot add method to reserved class [" + PainlessLookupUtility.DEF_CLASS_NAME + "]");
        }
        ...
        if (painlessClassBuilder == null) {
            throw new IllegalArgumentException("target class [" + targetCanonicalClassName + "] not found for method " +
                    "[[" + targetCanonicalClassName + "], [" + methodName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]");
        }
        ...
        for (Class<?> typeParameter : typeParameters) {
            if (isValidType(typeParameter) == false) {
                throw new IllegalArgumentException("type parameter [" + PainlessLookupUtility.typeToCanonicalTypeName(typeParameter) + "] " +
                        "not found for method [[" + targetCanonicalClassName + "], [" + methodName + "], " +
                        PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]");
            }
        ...
        if (isValidType(returnType) == false) {
            throw new IllegalArgumentException("return type [" + PainlessLookupUtility.typeToCanonicalTypeName(returnType) + "] not found for method " +
                    "[[" + targetCanonicalClassName + "], [" + methodName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]");
        }
        ...
        if (augmentedClass == null) {
            try {
                ...
                throw new IllegalArgumentException("reflection object not found for method [[" + targetCanonicalClassName + "], " +
                        "[" + methodName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]", nsme);
            }
        } else {
            try {
                ...
                if (Modifier.isStatic(javaMethod.getModifiers()) == false) {
                    throw new IllegalArgumentException("method [[" + targetCanonicalClassName + "], [" + methodName + "], " +
                            PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] with augmented class " +
                            "[" + PainlessLookupUtility.typeToCanonicalTypeName(augmentedClass) + "] must be static");
                }
            } catch (NoSuchMethodException nsme) {
                throw new IllegalArgumentException("reflection object not found for method " +
                        "[[" + targetCanonicalClassName + "], [" + methodName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] " +
                        "with augmented class [" + PainlessLookupUtility.typeToCanonicalTypeName(augmentedClass) + "]", nsme);
            }
        }
        ...
        if (javaMethod.getReturnType() != PainlessLookupUtility.typeToJavaType(returnType)) {
            throw new IllegalArgumentException("return type [" + PainlessLookupUtility.typeToCanonicalTypeName(javaMethod.getReturnType()) + "] " +
                    "does not match the specified returned type [" + PainlessLookupUtility.typeToCanonicalTypeName(returnType) + "] " +
        ...
        if (augmentedClass == null) {
            try {
                ...
                throw new IllegalArgumentException("method handle not found for method " +
                        ...
                        PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]", iae);
            }
        } else {
            try {
                ...
                throw new IllegalArgumentException("method handle not found for method " +
                        "[[" + targetClass.getCanonicalName() + "], [" + methodName + "], " +
                        PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]" +
                        "with augmented class [" + PainlessLookupUtility.typeToCanonicalTypeName(augmentedClass) + "]", iae);
            }
        }
        ...
        if (existingPainlessMethod == null) {
            ...
            throw new IllegalArgumentException("cannot add methods with the same name and arity but are not equivalent for methods " +
                    ...
                    PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] and " +
                    "[[" + targetCanonicalClassName + "], [" + methodName + "], " +
                    "[" + PainlessLookupUtility.typeToCanonicalTypeName(existingPainlessMethod.returnType) + "], " +
                    PainlessLookupUtility.typesToCanonicalTypeNames(existingPainlessMethod.typeParameters) + "]");
        }
    }
    ...
    public void addPainlessField(Class<?> targetClass, String fieldName, Class<?> typeParameter) {
        ...
        if (targetClass == def.class) {
            throw new IllegalArgumentException("cannot add field to reserved class [" + PainlessLookupUtility.DEF_CLASS_NAME + "]");
        }
        ...
        if (painlessClassBuilder == null) {
            throw new IllegalArgumentException("target class [" + targetCanonicalClassName + "] not found for field " +
                    "[[" + targetCanonicalClassName + "], [" + fieldName + "], [" + PainlessLookupUtility.typeToCanonicalTypeName(typeParameter) + "]]");
        }
        ...
        if (javaField.getType() != PainlessLookupUtility.typeToJavaType(typeParameter)) {
            throw new IllegalArgumentException("type parameter [" + PainlessLookupUtility.typeToCanonicalTypeName(javaField.getType()) + "] " +
                    "does not match the specified type parameter [" + PainlessLookupUtility.typeToCanonicalTypeName(typeParameter) + "] " +
                    "for field [[" + targetCanonicalClassName + "], [" + fieldName + "]");
        }
        ...
        if (Modifier.isStatic(javaField.getModifiers())) {
            ...
            if (existingPainlessField == null) {
                ...
                throw new IllegalArgumentException("cannot add fields with the same name but are not equivalent for fields " +
                        ...
                        PainlessLookupUtility.typeToCanonicalTypeName(typeParameter) + "] and " +
                        "[[" + targetCanonicalClassName + "], [" + existingPainlessField.javaField.getName() + "], " +
                        PainlessLookupUtility.typeToCanonicalTypeName(existingPainlessField.typeParameter) + "] " +
    ...
    public void addImportedPainlessMethod(Class<?> targetClass, String methodName, Class<?> returnType, List<Class<?>> typeParameters,
        ...
        if (targetClass == def.class) {
            throw new IllegalArgumentException("cannot add imported method from reserved class [" + PainlessLookupUtility.DEF_CLASS_NAME + "]");
        }
        ...
        if (isValidType(returnType) == false) {
            throw new IllegalArgumentException("return type [" + PainlessLookupUtility.typeToCanonicalTypeName(returnType) + "] not found for imported method " +
                    "[[" + targetCanonicalClassName + "], [" + methodName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("imported method reflection object [[" + targetCanonicalClassName + "], " +
                    "[" + methodName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] not found", nsme);
        }
        ...
        if (Modifier.isStatic(javaMethod.getModifiers()) == false) {
            throw new IllegalArgumentException("imported method [[" + targetClass.getCanonicalName() + "], [" + methodName + "], " +
                    PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] must be static");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("imported method handle [[" + targetClass.getCanonicalName() + "], " +
                    "[" + methodName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] not found", iae);
        }
        ...
        if (existingImportedPainlessMethod == null) {
            ...
            throw new IllegalArgumentException("cannot add imported methods with the same name and arity " +
                    ...
                    PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] and " +
                    "[[" + targetCanonicalClassName + "], [" + methodName + "], " +
                    "[" + PainlessLookupUtility.typeToCanonicalTypeName(existingImportedPainlessMethod.returnType) + "], " +
                    PainlessLookupUtility.typesToCanonicalTypeNames(existingImportedPainlessMethod.typeParameters) + "]");
        }
    }
    ...
    public void addPainlessClassBinding(Class<?> targetClass, String methodName, Class<?> returnType, List<Class<?>> typeParameters,
        ...
        if (targetClass == def.class) {
            throw new IllegalArgumentException("cannot add class binding as reserved class [" + PainlessLookupUtility.DEF_CLASS_NAME + "]");
        }
        ...
        for (int typeParameterIndex = 0; typeParameterIndex < constructorTypeParametersSize; ++typeParameterIndex) {
            ...
            if (isValidType(typeParameter) == false) {
                throw new IllegalArgumentException("type parameter [" + PainlessLookupUtility.typeToCanonicalTypeName(typeParameter) + "] not found " +
                        "for class binding [[" + targetCanonicalClassName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]");
            }
            ...
            if (javaTypeParameter != PainlessLookupUtility.typeToJavaType(typeParameter)) {
                throw new IllegalArgumentException("type parameter [" + PainlessLookupUtility.typeToCanonicalTypeName(javaTypeParameter) + "] " +
                        "does not match the specified type parameter [" + PainlessLookupUtility.typeToCanonicalTypeName(typeParameter) + "] " +
                        "for class binding [[" + targetClass.getCanonicalName() + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]");
            }
        }
        ...
        if (isValidType(returnType) == false) {
            throw new IllegalArgumentException("return type [" + PainlessLookupUtility.typeToCanonicalTypeName(returnType) + "] not found for class binding " +
                    "[[" + targetCanonicalClassName + "], [" + methodName + "], " + PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "]");
        }
        ...
        String painlessMethodKey = PainlessLookupUtility.buildPainlessMethodKey(methodName, constructorTypeParametersSize + methodTypeParametersSize);
        ...
        if (Modifier.isStatic(javaMethod.getModifiers())) {
            throw new IllegalArgumentException("class binding [[" + targetClass.getCanonicalName() + "], [" + methodName + "], " +
                    PainlessLookupUtility.typesToCanonicalTypeNames(typeParameters) + "] cannot be static");
        }
        ...
        if (existingPainlessClassBinding == null) {
            ...
            throw new IllegalArgumentException("cannot add class bindings with the same name and arity " +
                    ...
                    "[[" + targetCanonicalClassName + "], " +
                    "[" + methodName + "], " +
                    "[" + PainlessLookupUtility.typeToCanonicalTypeName(existingPainlessClassBinding.returnType) + "], " +
    ...
    public void addPainlessInstanceBinding(Object targetInstance, String methodName, Class<?> returnType, List<Class<?>> typeParameters) {
        ...
        if (targetClass == def.class) {
            throw new IllegalArgumentException("cannot add instance binding as reserved class [" + PainlessLookupUtility.DEF_CLASS_NAME + "]");
        }
        ...
        if (existingPainlessInstanceBinding == null) {
            ...
            throw new IllegalArgumentException("cannot add instances bindings with the same name and arity " +
                    ...
                    "[[" + targetCanonicalClassName + "], " +
                    "[" + methodName + "], " +
                    "[" + PainlessLookupUtility.typeToCanonicalTypeName(existingPainlessInstanceBinding.returnType) + "], " +
                    PainlessLookupUtility.typesToCanonicalTypeNames(existingPainlessInstanceBinding.typeParameters) + "]");
        }
    }
    ...
    private void setFunctionalInterfaceMethod(Class<?> targetClass, PainlessClassBuilder painlessClassBuilder) {
        if (targetClass.isInterface()) {
            ...
            if (javaMethods.size() != 1 && targetClass.isAnnotationPresent(FunctionalInterface.class)) {
                throw new IllegalArgumentException("class [" + PainlessLookupUtility.typeToCanonicalTypeName(targetClass) + "] " +
    ...
    private void generateBridgeMethod(PainlessClassBuilder painlessClassBuilder, PainlessMethod painlessMethod) {
        ...
        if (bridgePainlessMethod == null) {
            ...
            String bridgeClassName =
                    "org/renameme/painless/Bridge$" + javaMethod.getDeclaringClass().getSimpleName() + "$" + javaMethod.getName();
            ClassWriter bridgeClassWriter = new ClassWriter(bridgeClassFrames);
            bridgeClassWriter.visit(
                    WriterConstants.CLASS_VERSION, bridgeClassAccess, bridgeClassName, null, WriterConstants.OBJECT_TYPE.getInternalName(), null);
            ...
            bridgeConstructorWriter.invokeConstructor(WriterConstants.OBJECT_TYPE, bridgeConstructorType);
            ...
            for (int typeParameterCount = 0; typeParameterCount < javaMethod.getParameterTypes().length; ++typeParameterCount) {
                ...
                if      (typeParameter == Byte.class)      bridgeMethodWriter.invokeStatic(WriterConstants.DEF_UTIL_TYPE, WriterConstants.DEF_TO_B_BYTE_IMPLICIT);
                else if (typeParameter == Short.class)     bridgeMethodWriter.invokeStatic(WriterConstants.DEF_UTIL_TYPE, WriterConstants.DEF_TO_B_SHORT_IMPLICIT);
                else if (typeParameter == Character.class) bridgeMethodWriter.invokeStatic(WriterConstants.DEF_UTIL_TYPE, WriterConstants.DEF_TO_B_CHARACTER_IMPLICIT);
                else if (typeParameter == Integer.class)   bridgeMethodWriter.invokeStatic(WriterConstants.DEF_UTIL_TYPE, WriterConstants.DEF_TO_B_INTEGER_IMPLICIT);
                else if (typeParameter == Long.class)      bridgeMethodWriter.invokeStatic(WriterConstants.DEF_UTIL_TYPE, WriterConstants.DEF_TO_B_LONG_IMPLICIT);
                else if (typeParameter == Float.class)     bridgeMethodWriter.invokeStatic(WriterConstants.DEF_UTIL_TYPE, WriterConstants.DEF_TO_B_FLOAT_IMPLICIT);
                else if (typeParameter == Double.class)    bridgeMethodWriter.invokeStatic(WriterConstants.DEF_UTIL_TYPE, WriterConstants.DEF_TO_B_DOUBLE_IMPLICIT);
            }
