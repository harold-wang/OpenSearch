...
public interface EvaluationMetric extends ToXContentObject, NamedWriteable {
     ...
     */
    static List<RatedDocument.DocumentKey> filterUnratedDocuments(List<RatedSearchHit> ratedHits) {
        return ratedHits.stream().filter(hit -> hit.getRating().isPresent() == false)
                .map(hit -> new RatedDocument.DocumentKey(hit.getSearchHit().getIndex(), hit.getSearchHit().getId())).collect(Collectors.toList());
    }
