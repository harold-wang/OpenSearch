...
import org.renameme.common.util.SingleObjectCache;
    ...
    protected List<TransportAddress> fetchDynamicNodes() {
        ...
        for (final Reservation reservation : descInstances.getReservations()) {
            for (final Instance instance : reservation.getInstances()) {
                ...
                String address = null;
                if (hostType.equals(AwsEc2Service.HostType.PRIVATE_DNS)) {
                    address = instance.getPrivateDnsName();
                } else if (hostType.equals(AwsEc2Service.HostType.PRIVATE_IP)) {
                    address = instance.getPrivateIpAddress();
                } else if (hostType.equals(AwsEc2Service.HostType.PUBLIC_DNS)) {
                    address = instance.getPublicDnsName();
                } else if (hostType.equals(AwsEc2Service.HostType.PUBLIC_IP)) {
                    address = instance.getPublicIpAddress();
                } else if (hostType.startsWith(AwsEc2Service.HostType.TAG_PREFIX)) {
                    // Reading the node host from its metadata
                    final String tagName = hostType.substring(AwsEc2Service.HostType.TAG_PREFIX.length());
