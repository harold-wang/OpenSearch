...
package org.renameme.discovery.azure.classic;
...
import org.renameme.cloud.azure.classic.AzureServiceDisableException;
import org.renameme.cloud.azure.classic.AzureServiceRemoteException;
import org.renameme.cloud.azure.classic.management.AzureComputeService;
...
public class AzureSeedHostsProvider implements SeedHostsProvider {
    ...
    public AzureSeedHostsProvider(Settings settings, AzureComputeService azureComputeService,
        ...
        this.refreshInterval = AzureComputeService.Discovery.REFRESH_SETTING.get(settings);

        this.hostType = AzureComputeService.Discovery.HOST_TYPE_SETTING.get(settings);
        this.publicEndpointName = AzureComputeService.Discovery.ENDPOINT_NAME_SETTING.get(settings);

        // Deployment name could be set with discovery.azure.deployment.name
        // Default to cloud.azure.management.cloud.service.name
        this.deploymentName = AzureComputeService.Discovery.DEPLOYMENT_NAME_SETTING.get(settings);

        // Reading deployment_slot
        this.deploymentSlot = AzureComputeService.Discovery.DEPLOYMENT_SLOT_SETTING.get(settings).slot;
    }
