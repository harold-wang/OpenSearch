...
package org.renameme.discovery.gce;

import org.renameme.Version;
import org.renameme.cloud.gce.GceInstancesServiceImpl;
import org.renameme.cloud.gce.GceMetadataService;
 ...
 * for a given project-id and zone under the src/test/resources/org/renameme/discovery/gce with dir name:
 *
 ...
 *  .build();
 *
 *  You need to create a file under `src/test/resources/org/renameme/discovery/gce/` named:
 *
...
public class GceDiscoveryTests extends ESTestCase {
    ...
    public void testNodesWithDifferentTagsAndOneTagSet() {
        Settings nodeSettings = Settings.builder()
                .put(GceInstancesServiceImpl.PROJECT_SETTING.getKey(), projectName)
                .put(GceInstancesServiceImpl.ZONE_SETTING.getKey(), "europe-west1-b")
                .putList(GceSeedHostsProvider.TAGS_SETTING.getKey(), "renameme")
                .build();
        mock = new GceInstancesServiceMock(nodeSettings);
        List<TransportAddress> dynamicHosts = buildDynamicNodes(mock, nodeSettings);
    ...
    public void testNodesWithDifferentTagsAndTwoTagSet() {
        Settings nodeSettings = Settings.builder()
                .put(GceInstancesServiceImpl.PROJECT_SETTING.getKey(), projectName)
                .put(GceInstancesServiceImpl.ZONE_SETTING.getKey(), "europe-west1-b")
                .putList(GceSeedHostsProvider.TAGS_SETTING.getKey(), "renameme", "dev")
                .build();
        mock = new GceInstancesServiceMock(nodeSettings);
        List<TransportAddress> dynamicHosts = buildDynamicNodes(mock, nodeSettings);
        assertThat(dynamicHosts, hasSize(1));
