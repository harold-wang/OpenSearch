...
import org.renameme.gradle.test.GradleUnitTestCase;
...
public class DistributionDownloadPluginTests extends GradleUnitTestCase {
    ...
    public void testVersionDefault() {
        ...
        assertEquals(distro.getVersion(), VersionProperties.getRenameme());
    }
    ...
    public void testPlatformForIntegTest() {
        assertDistroError(
            ...
            null,
            null,
            "platform cannot be set on renameme distribution [testdistro]"
        );
    }
    ...
    public void testFlavorForIntegTest() {
        assertDistroError(
            ...
            Flavor.OSS,
            null,
            "flavor [oss] not allowed for renameme distribution [testdistro] of type [integ_test_zip]"
        );
    }
    ...
    public void testBundledJdkForIntegTest() {
        assertDistroError(
            ...
            null,
            true,
            "bundledJdk cannot be set on renameme distribution [testdistro]"
        );
    }
    ...
    public void testLocalCurrentVersionIntegTestZip() {
        ...
        createDistro(project, "distro", VersionProperties.getRenameme(), Type.INTEG_TEST_ZIP, null, null, null);
        checkPlugin(project);
    }
        ...
        for (Platform platform : Platform.values()) {
            for (Flavor flavor : Flavor.values()) {
                for (boolean bundledJdk : new boolean[] { true, false }) {
                    ...
                    archiveProject.getConfigurations().create("default");
                    archiveProject.getArtifacts().add("default", new File("doesnotmatter"));
                    createDistro(project, "distro", VersionProperties.getRenameme(), Type.ARCHIVE, platform, flavor, bundledJdk);
                    checkPlugin(project);
                }
            }
        }
        ...
        for (Type packageType : new Type[] { Type.RPM, Type.DEB }) {
            for (Flavor flavor : Flavor.values()) {
                for (boolean bundledJdk : new boolean[] { true, false }) {
                    ...
                    createDistro(project, "distro", VersionProperties.getRenameme(), packageType, null, flavor, bundledJdk);
                    checkPlugin(project);
                }
            }
        }
    ...
    private RenamemeDistribution createDistro(
        Project project,
        String name,
        String version,
        Type type,
        ...
        NamedDomainObjectContainer<RenamemeDistribution> distros = DistributionDownloadPlugin.getContainer(project);
    ...
    private RenamemeDistribution checkDistro(
        Project project,
        String name,
        String version,
        Type type,
        ...
        RenamemeDistribution distribution = createDistro(project, name, version, type, platform, flavor, bundledJdk);
    ...
    private Project createProject(BwcVersions bwcVersions, boolean isInternal) {
        ...
        project.getPlugins().apply("renameme.distribution-download");
        return project;
    }
