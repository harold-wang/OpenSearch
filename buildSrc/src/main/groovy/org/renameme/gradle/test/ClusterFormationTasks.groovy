 ...
 * under the License.
 */
package org.renameme.gradle.test

import org.apache.tools.ant.DefaultLogger
import org.apache.tools.ant.taskdefs.condition.Os
import org.renameme.gradle.BuildPlugin
import org.renameme.gradle.BwcVersions
import org.renameme.gradle.LoggedExec
...
import org.renameme.gradle.plugin.PluginBuildPlugin
import org.renameme.gradle.plugin.PluginPropertiesExtension
...
class ClusterFormationTasks {
    ...
    static List<NodeInfo> setup(Project project, String prefix, Task runner, ClusterConfiguration config) {
        ...
        Configuration currentDistro = project.configurations.create("${prefix}_renamemeDistro")
        Configuration bwcDistro = project.configurations.create("${prefix}_renamemeBwcDistro")
        Configuration bwcPlugins = project.configurations.create("${prefix}_renamemeBwcPlugins")
        ...
        configureDistributionDependency(project, config.distribution, currentDistro, VersionProperties.renameme)
        ...
        for (int i = 0; i < config.numNodes; i++) {
            ...
            String renamemeVersion
            if (i < config.numBwcNodes) {
                renamemeVersion = config.bwcVersion.toString()
                if (project.bwcVersions.unreleased.contains(config.bwcVersion)) {
                    renamemeVersion += "-SNAPSHOT"
                }
                distro = bwcDistro
            } else {
                renamemeVersion = VersionProperties.renameme
                distro = currentDistro
            }
            NodeInfo node = new NodeInfo(config, i, project, prefix, renamemeVersion, sharedDir)
            nodes.add(node)
    ...
    static void configureDistributionDependency(Project project, String distro, Configuration configuration, String renamemeVersion) {
        ...
        if (distro.equals("integ-test-zip")) {
            ...
            if (internalBuild) {
                ...
                project.dependencies.add(
                        configuration.name,
                        "org.renameme.distribution.integ-test-zip:renameme:${renamemeVersion}@zip"
                )
            }
            return
        }
        ...
        Version version = Version.fromString(renamemeVersion)
        String os = getOs()
        String classifier = "-${os}-x86_64"
        String packaging = os.equals('windows') ? 'zip' : 'tar.gz'
        String artifactName = 'renameme'
        if (distro.equals('oss') && Version.fromString(renamemeVersion).onOrAfter('6.3.0')) {
            artifactName += '-oss'
        }
        Object dependency
        ...
        if (unreleasedInfo != null) {
        ...
        } else if (internalBuild && renamemeVersion.equals(VersionProperties.renameme)) {
            ...
            dependency = "dnm:${artifactName}:${renamemeVersion}${classifier}@${packaging}"
        }
        project.dependencies.add(configuration.name, dependency)
    }

    /** Adds a dependency on a different version of the given plugin, which will be retrieved using gradle's dependency resolution */
    static void configureBwcPluginDependency(Project project, Object plugin, Configuration configuration, Version renamemeVersion) {
        if (plugin instanceof Project) {
            Project pluginProject = (Project)plugin
            verifyProjectHasBuildPlugin(configuration.name, renamemeVersion, project, pluginProject)
            final String pluginName = findPluginName(pluginProject)
            project.dependencies.add(configuration.name, "org.renameme.plugin:${pluginName}:${renamemeVersion}@zip")
        } else {
    ...
    /**
     * Adds dependent tasks to start an renameme cluster before the given task is executed,
     ...
     *   <li>Extract a fresh copy of renameme</li>
     *   <li>Write an renameme.yml config file</li>
    ...
    static Task configureNode(Project project, String prefix, Task runner, Object dependsOn, NodeInfo node, ClusterConfiguration config,
        ...
        if (node.config.plugins.isEmpty() == false) {
            if (node.nodeVersion == Version.fromString(VersionProperties.renameme)) {
    ...
    /** Adds a task to extract the renameme distribution */
    static Task configureExtractTask(String name, Project project, Task setup, NodeInfo node,
        ...
        /* configuration.singleFile will be an external artifact if this is being run by a plugin not living in the
          renameme source tree. If this is a plugin built in the renameme source tree or this is a distro in
          the renameme source tree then this should be the version of renameme built by the source tree.
    ...
    /** Adds a task to write renameme.yml for the given node configuration */
    static Task configureWriteConfigTask(String name, Project project, Task setup, NodeInfo node, Closure<Map> configFilter) {
        ...
        writeConfig.doFirst {
            ...
            File configFile = new File(node.pathConf, 'renameme.yml')
    ...
    static Task configureCreateKeystoreTask(String name, Project project, Task setup, NodeInfo node) {
        if (node.config.keystoreSettings.isEmpty() && node.config.keystoreFiles.isEmpty()) {
             ...
             */
            final Object esKeystoreUtil = "${-> node.binPath().resolve('renameme-keystore').toString()}"
    ...
    static Task configureCopyBwcPluginsTask(String name, Project project, Task setup, NodeInfo node, String prefix) {
        Configuration bwcPlugins = project.configurations.getByName("${prefix}_renamemeBwcPlugins")
        for (Map.Entry<String, Object> plugin : node.config.plugins.entrySet()) {
    ...
    static Task configureInstallPluginTask(String name, Project project, Task setup, NodeInfo node, String pluginName, String prefix) {
         ...
         * the short name requiring the path to already exist.
         */
        final Object esPluginUtil = "${-> node.binPath().resolve('renameme-plugin').toString()}"
    ...
    /** Adds a task to start an renameme node with the given configuration */
    static Task configureStartTask(String name, Project project, Task setup, NodeInfo node) {
        ...
        Closure antRunner = { AntBuilder ant ->
            ant.exec(executable: node.executable, spawn: node.config.daemonize, newenvironment: true,
                     dir: node.cwd, taskname: 'renameme') {
        ...
        // this closure is the actual code to run renameme
        Closure renamemeRunner = {
            // Due to how ant exec works with the spawn option, we lose all stdout/stderr from the
            // process executed. To work around this, when spawning, we wrap the renameme start
            // command inside another shell script, which simply internally redirects the output
            // of the real renameme script. This allows ant to keep the streams open with the
            // dummy process, but us to have the output available if there is an error in the
            // renameme start script
            if (node.config.daemonize) {
        ...
        start.doFirst {
            ...
            if (node.config.debug) {
                println 'Running renameme in debug mode, suspending until connected on port 8000'
    ...
    static Task configureWaitTask(String name, Project project, List<NodeInfo> nodes, List<Task> startTasks, int waitSeconds) {
        ...
        wait.doLast {
            ...
            if (ant.properties.containsKey("failed${name}".toString())) {
                waitFailed(project, nodes, logger, "Failed to start renameme: timed out after ${waitSeconds} seconds")
            }

            boolean anyNodeFailed = false
            for (NodeInfo node : nodes) {
                if (node.failedMarker.exists()) {
                    logger.error("Failed to start renameme: ${node.failedMarker.toString()} exists")
                    anyNodeFailed = true
                }
            }
            if (anyNodeFailed) {
                waitFailed(project, nodes, logger, 'Failed to start renameme')
            }
            ...
            if (missingFile) {
                waitFailed(project, nodes, logger, 'Renameme did not complete startup in time allotted')
            }
            ...
            for (NodeInfo node : nodes) {
                ...
                if (success == false) {
                    waitFailed(project, nodes, logger, 'Renameme cluster failed to pass wait condition')
                }
            }
        }
        return wait
    ...
    /** Adds a task to check if the process with the given pidfile is actually renameme */
    static Task configureCheckPreviousTask(String name, Project project, Object depends, NodeInfo node) {
        return project.tasks.create(name: name, type: Exec, dependsOn: depends) {
            ...
            doLast {
                ...
                if (out.contains("${ext.pid} org.renameme.bootstrap.Renameme") == false) {
                    logger.error('jps -l')
                    logger.error(out)
                    logger.error("pid file: ${node.pidFile}")
                    logger.error("pid: ${ext.pid}")
                    throw new GradleException("jps -l did not report any process with org.renameme.bootstrap.Renameme\n" +
    ...
    /** Adds a task to kill an renameme node with the given pidfile */
