...
import org.renameme.gradle.RenamemeJavaPlugin
import org.renameme.gradle.ExportRenamemeBuildResourcesTask
import org.renameme.gradle.RepositoriesSetupPlugin
import org.renameme.gradle.info.BuildParams
import org.renameme.gradle.info.GlobalBuildInfoPlugin
import org.renameme.gradle.precommit.PrecommitTasks
import org.renameme.gradle.testclusters.TestClustersPlugin
...
/**
 * Configures the build to compile tests against Renameme's test framework
...
class StandaloneRestTestPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        if (project.pluginManager.hasPlugin('renameme.build')) {
            throw new InvalidUserDataException('renameme.standalone-test '
                + 'renameme.standalone-rest-test, and renameme.build '
        ...
        project.getTasks().register("buildResources", ExportRenamemeBuildResourcesTask)
        RenamemeJavaPlugin.configureInputNormalization(project)
        RenamemeJavaPlugin.configureCompile(project)
