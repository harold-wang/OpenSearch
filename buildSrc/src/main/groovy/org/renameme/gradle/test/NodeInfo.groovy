...
class NodeInfo {
    ...
    /** a file written by renameme containing the ports of each bound address for http */
    File httpPortsFile

    /** a file written by renameme containing the ports of each bound address for transport */
    ...
    /** stdout/stderr log of the renameme process for this node */
    File startLog
    ...
    /** Executable to run the bin/renameme with, either cmd or sh */
    String executable

    /** Path to the renameme start script */
    ...
    /** the version of renameme that this node runs */
    Version nodeVersion
    ...
    NodeInfo(ClusterConfiguration config, int nodeNum, Project project, String prefix, String nodeVersion, File sharedDir) {
        ...
        this.isBwcNode = this.nodeVersion.before(VersionProperties.renameme)
        homeDir = new File(baseDir, "renameme-${nodeVersion}")
        ...
        configFile = new File(pathConf, 'renameme.yml')
        ...
        if (Os.isFamily(Os.FAMILY_WINDOWS)) {
             ...
             */
            esScript = "${-> binPath().resolve('renameme.bat').toString()}"
        } else {
            executable = 'bash'
            wrapperScript = new File(cwd, "run")
            esScript = binPath().resolve('renameme')
        }
        if (config.daemonize) {
            if (Os.isFamily(Os.FAMILY_WINDOWS)) {
                /*
