...
package org.renameme.gradle.vagrant;

import org.apache.commons.io.output.TeeOutputStream;
import org.renameme.gradle.LoggedExec;
import org.renameme.gradle.LoggingOutputStream;
...
public class VagrantMachine {
    ...
    // convert the given path from an renameme repo path to a VM path
    public static String convertLinuxPath(Project project, String path) {
        return "/renameme/" + project.getRootDir().toPath().relativize(Paths.get(path));
    }

    public static String convertWindowsPath(Project project, String path) {
        return "C:\\renameme\\" + project.getRootDir().toPath().relativize(Paths.get(path)).toString().replace('/', '\\');
    }
