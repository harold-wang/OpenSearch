...
import org.renameme.gradle.RenamemeDistribution.Flavor;
import org.renameme.gradle.RenamemeDistribution.Platform;
import org.renameme.gradle.RenamemeDistribution.Type;
import org.renameme.gradle.Jdk;
import org.renameme.gradle.JdkDownloadPlugin;
import org.renameme.gradle.SystemPropertyCommandLineArgumentProvider;
import org.renameme.gradle.Version;
import org.renameme.gradle.VersionProperties;
import org.renameme.gradle.docker.DockerSupportPlugin;
import org.renameme.gradle.docker.DockerSupportService;
import org.renameme.gradle.info.BuildParams;
import org.renameme.gradle.internal.InternalDistributionDownloadPlugin;
import org.renameme.gradle.util.GradleUtils;
import org.renameme.gradle.util.Util;
import org.renameme.gradle.vagrant.VagrantBasePlugin;
import org.renameme.gradle.vagrant.VagrantExtension;
...
import static org.renameme.gradle.vagrant.VagrantMachine.convertLinuxPath;
import static org.renameme.gradle.vagrant.VagrantMachine.convertWindowsPath;
...
public class DistroTestPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        project.getPluginManager().apply("renameme.build");
        ...
        NamedDomainObjectContainer<RenamemeDistribution> allDistributions = DistributionDownloadPlugin.getContainer(project);
        List<RenamemeDistribution> testDistributions = configureDistributions(project);

        Map<RenamemeDistribution.Type, TaskProvider<?>> lifecycleTasks = lifecycleTasks(project, "destructiveDistroTest");
        ...
        for (RenamemeDistribution distribution : testDistributions) {
            ...
            if ((distribution.getType() == Type.DEB || distribution.getType() == Type.RPM) && distribution.getBundledJdk()) {
                for (Version version : BuildParams.getBwcVersions().getIndexCompatible()) {
                    ...
                    final RenamemeDistribution bwcDistro;
        ...
        project.subprojects(vmProject -> {
            ...
            Map<RenamemeDistribution.Type, TaskProvider<?>> vmLifecyleTasks = lifecycleTasks(vmProject, "distroTest");
    ...
    private static Map<RenamemeDistribution.Type, TaskProvider<?>> lifecycleTasks(Project project, String taskPrefix) {
        Map<RenamemeDistribution.Type, TaskProvider<?>> lifecyleTasks = new HashMap<>();
    ...
    private static TaskProvider<Test> configureTestTask(
        Project project,
        String taskname,
        RenamemeDistribution distribution,
    ...
    private List<RenamemeDistribution> configureDistributions(Project project) {
        NamedDomainObjectContainer<RenamemeDistribution> distributions = DistributionDownloadPlugin.getContainer(project);
        List<RenamemeDistribution> currentDistros = new ArrayList<>();
            ...
            for (Type type : Arrays.asList(Type.DEB, Type.RPM, Type.DOCKER, Type.DOCKER_UBI)) {
                for (Flavor flavor : Flavor.values()) {
                    for (boolean bundledJdk : Arrays.asList(true, false)) {
                        ...
                        currentDistros.add(
                            createDistro(distributions, architecture, type, null, flavor, bundledJdk, VersionProperties.getRenameme())
                        );
                    }
                }
            }
    ...
    private static RenamemeDistribution createDistro(
        NamedDomainObjectContainer<RenamemeDistribution> distributions,
        ...
        RenamemeDistribution distro = distributions.create(name, d -> {
    ...
    private static String destructiveDistroTestTaskName(RenamemeDistribution distro) {
    ...
    private static String destructiveDistroUpgradeTestTaskName(RenamemeDistribution distro, String bwcVersion) {
        Type type = distro.getType();
