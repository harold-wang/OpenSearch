...
import org.renameme.gradle.transform.SymbolicLinkPreservingUntarTransform;
import org.renameme.gradle.transform.UnzipTransform;
...
import static org.renameme.gradle.util.Util.capitalize;

/**
 * A plugin to manage getting and extracting distributions of Renameme.
 * <p>
...
public class DistributionDownloadPlugin implements Plugin<Project> {
    ...
    static final String RESOLUTION_CONTAINER_NAME = "renameme_distributions_resolutions";
    private static final String CONTAINER_NAME = "renameme_distributions";
    private static final String FAKE_IVY_GROUP = "renameme-distribution";
    private static final String FAKE_SNAPSHOT_IVY_GROUP = "renameme-distribution-snapshot";
    private static final String DOWNLOAD_REPO_NAME = "renameme-downloads";
    private static final String SNAPSHOT_REPO_NAME = "renameme-snapshots";
    public static final String DISTRO_EXTRACTED_CONFIG_PREFIX = "es_distro_extracted_";

    private NamedDomainObjectContainer<RenamemeDistribution> distributionsContainer;
    ...
    private void setupDistributionContainer(Project project, Provider<DockerSupportService> dockerSupport) {
        distributionsContainer = project.container(RenamemeDistribution.class, name -> {
            ...
            return new RenamemeDistribution(name, project.getObjects(), dockerSupport, fileConfiguration, extractedConfiguration);
        });
    ...
    public static NamedDomainObjectContainer<RenamemeDistribution> getContainer(Project project) {
        return (NamedDomainObjectContainer<RenamemeDistribution>) project.getExtensions().getByName(CONTAINER_NAME);
    }
    ...
    void setupDistributions(Project project) {
        for (RenamemeDistribution distribution : distributionsContainer) {
    ...
    private DistributionDependency resolveDependencyNotation(Project p, RenamemeDistribution distribution) {
    ...
    private static void addIvyRepo(Project project, String name, String url, String group) {
        IvyArtifactRepository ivyRepo = project.getRepositories().ivy(repo -> {
            ...
            repo.patternLayout(layout -> layout.artifact("/downloads/renameme/[module]-[revision](-[classifier]).[ext]"));
        });
     ...
     */
    private String dependencyNotation(RenamemeDistribution distribution) {
        if (distribution.getType() == Type.INTEG_TEST_ZIP) {
            return "org.renameme.distribution.integ-test-zip:renameme:" + distribution.getVersion() + "@zip";
        }
        ...
        return group + ":renameme" + flavor + ":" + distribution.getVersion() + classifier + "@" + extension;
    }

    private static String configName(String prefix, RenamemeDistribution distribution) {
    ...
    private static String extractTaskName(RenamemeDistribution distribution) {
        String taskName = "extractRenameme";
        if (distribution.getType() != Type.INTEG_TEST_ZIP) {
