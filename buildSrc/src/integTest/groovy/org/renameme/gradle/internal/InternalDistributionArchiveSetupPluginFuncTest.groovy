...
class InternalDistributionArchiveSetupPluginFuncTest extends AbstractGradleFuncTest {
    ...
    def setup() {
        buildFile << """
        import org.renameme.gradle.tar.SymbolicLinkPreservingTar

        plugins {
            id 'renameme.internal-distribution-archive-setup'
        }
        """
    ...
    def "applies defaults to tar tasks"() {
        ...
        "buildDarwinTar"    | "darwin-tar/build/distributions/renameme.tar.gz"
        "buildOssDarwinTar" | "oss-darwin-tar/build/distributions/renameme-oss.tar.gz"
    }
    ...
    def "applies defaults to zip tasks"() {
        ...
        where:
        buildTaskName       | expectedOutputArchivePath
        "buildDarwinZip"    | "darwin-zip/build/distributions/renameme.zip"
        "buildOssDarwinZip" | "oss-darwin-zip/build/distributions/renameme-oss.zip"
    }
    ...
    def "registered distribution provides archives and directory variant"() {
        ...
        file("producer-tar/build/distributions/renameme.tar.gz").exists()
        file("consumer/build/archives/renameme.tar.gz").exists()
