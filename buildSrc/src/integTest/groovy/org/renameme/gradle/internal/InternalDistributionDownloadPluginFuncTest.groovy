...
class InternalDistributionDownloadPluginFuncTest extends AbstractGradleFuncTest {
    ...
    def "plugin application fails on non internal build"() {
        ...
        buildFile.text = """
            plugins {
             id 'renameme.internal-distribution-download'
            }
        """
        ...
        then:
        assertOutputContains(result.output, "Plugin 'renameme.internal-distribution-download' is not supported. " +
    ...
    def "resolves current version from local build"() {
        ...
        internalBuild()
        localDistroSetup()
        def distroVersion = VersionProperties.getRenameme()
        buildFile << """
            apply plugin: 'renameme.internal-distribution-download'
    ...
    def "fails on resolving bwc versions with no bundled jdk"() {
        ...
        buildFile << """
            ...
            tasks.register("createExtractedTestDistro") {
                dependsOn renameme_distributions.test_distro.extracted
            }
        """
        when:
