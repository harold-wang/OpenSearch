    ...
    def setup() {
        ["darwin-zip", 'darwin-tar'].each { projName ->
            ...
            file("${projName}/build.gradle") << """
                plugins {
                  id 'renameme.internal-distribution-archive-check'
                }"""
        }
        file("SomeFile.txt") << """
            some dummy txt file
...
Copyright 2009-2018 Acme Coorp"""
        buildFile << """
            apply plugin:'base'
            tasks.withType(AbstractArchiveTask).configureEach {
                into("renameme-${VersionProperties.getRenameme()}") {
                    from 'LICENSE.txt'
                    from 'SomeFile.txt'
                    from 'NOTICE.txt'
                }
        ...
        normalizedOutput(result.output).contains("> expected line [2] in " +
                "[./darwin-tar/build/tar-extracted/renameme-${VersionProperties.getRenameme()}/NOTICE.txt] " +
