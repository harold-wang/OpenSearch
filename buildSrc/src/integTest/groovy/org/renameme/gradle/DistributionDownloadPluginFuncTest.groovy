...
import org.renameme.gradle.fixtures.AbstractGradleFuncTest
import org.renameme.gradle.transform.SymbolicLinkPreservingUntarTransform
import org.gradle.testkit.runner.TaskOutcome
import spock.lang.Unroll

import static org.renameme.gradle.fixtures.DistributionDownloadFixture.withMockedDistributionDownload
...
class DistributionDownloadPluginFuncTest extends AbstractGradleFuncTest {
    ...
    def "#distType version can be resolved"() {
        ...
        VersionProperties.getRenameme()      | RenamemeDistribution.Platform.LINUX        | "current"
        "8.1.0-SNAPSHOT"                     | RenamemeDistribution.Platform.LINUX        | "bwc"
        "7.0.0"                              | RenamemeDistribution.Platform.WINDOWS      | "released"
    }


    def "transformed versions are kept across builds"() {
        given:
        def version = VersionProperties.getRenameme()
        def platform = RenamemeDistribution.Platform.LINUX
    ...
    def "transforms are reused across projects"() {
        ...
        buildFile.text = """
            ...
            plugins {
                id 'renameme.distribution-download'
            }

            subprojects {
                apply plugin: 'renameme.distribution-download'
        ...
        then:
        result.tasks.size() == 3
        result.output.count("Unpacking renameme-oss-${version}-linux-x86_64.tar.gz " +
    ...
    private boolean assertExtractedDistroCreated(String relativePath) {
        ...
        assert new File(distroExtracted, "renameme-1.2.3/bin/renameme").exists()
        true
    }

    private static String applyPluginAndSetupDistro(String version, RenamemeDistribution.Platform platform) {
        """
            import org.renameme.gradle.Architecture
    ...
    private static String setupTestDistro(String version, RenamemeDistribution.Platform platform) {
        return """
            renameme_distributions {
                test_distro {
                    version = "$version"
                    type = "archive"
                    platform = "$platform"
    ...
    private static String setupDistroTask() {
        return """
            tasks.register("setupDistro", Sync) {
                from(renameme_distributions.test_distro.extracted)
                into("build/distro")
            }
            """
    }
