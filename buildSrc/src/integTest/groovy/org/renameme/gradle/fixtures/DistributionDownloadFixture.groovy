...
package org.renameme.gradle.fixtures
...
class DistributionDownloadFixture {
    ...
    static BuildResult withMockedDistributionDownload(GradleRunner gradleRunner, Closure<BuildResult> buildRunClosure) {
        return withMockedDistributionDownload(VersionProperties.getRenameme(), RenamemeDistribution.CURRENT_PLATFORM,
                gradleRunner, buildRunClosure)
    }

    static BuildResult withMockedDistributionDownload(String version, RenamemeDistribution.Platform platform,
    ...
    private static String urlPath(String version,RenamemeDistribution.Platform platform) {
        String fileType = ((platform == RenamemeDistribution.Platform.LINUX ||
                platform == RenamemeDistribution.Platform.DARWIN)) ? "tar.gz" : "zip"
        "/downloads/renameme/renameme-oss-${version}-${platform}-x86_64.$fileType"
    }

    private static byte[] filebytes(String urlPath) throws IOException {
        String suffix = urlPath.endsWith("zip") ? "zip" : "tar.gz";
        return DistributionDownloadFixture.getResourceAsStream("/org/renameme/gradle/fake_renameme." + suffix).getBytes()
    }
}
