...
class TestClustersPluginFuncTest extends AbstractGradleFuncTest {
    ...
    def setup() {
        buildFile << """
            import org.renameme.gradle.testclusters.DefaultTestClustersTask
            plugins {
                id 'renameme.testclusters'
            }
    ...
    def "test cluster distribution is configured and started"() {
        ...
        then:
        result.output.contains("renameme-keystore script executed!")
        assertEsStdoutContains("myCluster", "Starting Renameme process")
