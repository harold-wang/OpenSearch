...
public class RenamemeNodesSnifferTests extends RestClientTestCase {

    private int sniffRequestTimeout;
    private RenamemeNodesSniffer.Scheme scheme;
    ...
    public void startHttpServer() throws IOException {
        ...
        this.scheme = RandomPicks.randomFrom(getRandom(), RenamemeNodesSniffer.Scheme.values());
        if (rarely()) {
    ...
    public void testConstructorValidation() throws IOException {
        try {
            new RenamemeNodesSniffer(null, 1, RenamemeNodesSniffer.Scheme.HTTP);
        ...
        try (RestClient restClient = RestClient.builder(httpHost).build()) {
            try {
                new RenamemeNodesSniffer(restClient, 1, null);
                fail("should have failed");
            } catch (NullPointerException e) {
                assertEquals(e.getMessage(), "scheme cannot be null");
            }
            try {
                new RenamemeNodesSniffer(restClient, RandomNumbers.randomIntBetween(getRandom(), Integer.MIN_VALUE, 0),
                    RenamemeNodesSniffer.Scheme.HTTP);
                fail("should have failed");
    ...
    public void testSniffNodes() throws IOException {
        HttpHost httpHost = new HttpHost(httpServer.getAddress().getHostString(), httpServer.getAddress().getPort());
        try (RestClient restClient = RestClient.builder(httpHost).build()) {
            RenamemeNodesSniffer sniffer = new RenamemeNodesSniffer(restClient, sniffRequestTimeout, scheme);
            try {
    ...
    private static SniffResponse buildSniffResponse(RenamemeNodesSniffer.Scheme scheme) throws IOException {
        ...
        if (getRandom().nextBoolean()) {
            generator.writeStringField("cluster_name", "renameme");
        }
        if (getRandom().nextBoolean()) {
