...
package org.renameme.client.sniff;
...
import org.renameme.client.Node.Roles;
...
/**
 * Class responsible for sniffing the http hosts from renameme through the nodes info api and returning them back.
 * Compatible with renameme 2.x+.
 */
public final class RenamemeNodesSniffer implements NodesSniffer {

    private static final Log logger = LogFactory.getLog(RenamemeNodesSniffer.class);
    ...
    /**
     * Creates a new instance of the Renameme sniffer. It will use the provided {@link RestClient} to fetch the hosts,
     ...
     * @param restClient client used to fetch the hosts from renameme through nodes info api. Usually the same instance
     ...
     */
    public RenamemeNodesSniffer(RestClient restClient) {
        this(restClient, DEFAULT_SNIFF_REQUEST_TIMEOUT, RenamemeNodesSniffer.Scheme.HTTP);
    }
     ...
     *                                  to renameme. Allows to halt the request without any failure, as only the nodes
     *                                  that have responded within this timeout will be returned.
     * @param scheme the scheme to associate sniffed nodes with (as it is not returned by renameme)
     */
    public RenamemeNodesSniffer(RestClient restClient, long sniffRequestTimeoutMillis, Scheme scheme) {
    ...
    /**
     * Calls the renameme nodes info api, parses the response and returns all the found http hosts
     */
    @Override
    ...
    private static Node readNode(String nodeId, JsonParser parser, Scheme scheme) throws IOException {
         ...
         * address on which it is listening. This is useful in Renameme's
