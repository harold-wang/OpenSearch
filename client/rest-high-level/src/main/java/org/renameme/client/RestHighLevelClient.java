...
import org.renameme.client.core.CountRequest;
import org.renameme.client.core.CountResponse;
import org.renameme.client.core.GetSourceRequest;
import org.renameme.client.core.GetSourceResponse;
import org.renameme.client.core.MainRequest;
import org.renameme.client.core.MainResponse;
import org.renameme.client.core.MultiTermVectorsRequest;
import org.renameme.client.core.MultiTermVectorsResponse;
import org.renameme.client.core.TermVectorsRequest;
import org.renameme.client.core.TermVectorsResponse;
import org.renameme.client.tasks.TaskSubmissionResponse;
import org.renameme.common.CheckedConsumer;
import org.renameme.common.CheckedFunction;
import org.renameme.common.ParseField;
import org.renameme.common.xcontent.ContextParser;
import org.renameme.common.xcontent.DeprecationHandler;
...
import org.renameme.index.rankeval.RankEvalRequest;
import org.renameme.index.rankeval.RankEvalResponse;
import org.renameme.index.reindex.BulkByScrollResponse;
import org.renameme.index.reindex.DeleteByQueryRequest;
import org.renameme.index.reindex.ReindexRequest;
import org.renameme.index.reindex.UpdateByQueryRequest;
import org.renameme.plugins.spi.NamedXContentProvider;
import org.renameme.rest.BytesRestResponse;
import org.renameme.rest.RestStatus;
import org.renameme.script.mustache.MultiSearchTemplateRequest;
import org.renameme.script.mustache.MultiSearchTemplateResponse;
import org.renameme.script.mustache.SearchTemplateRequest;
import org.renameme.script.mustache.SearchTemplateResponse;
import org.renameme.search.aggregations.Aggregation;
import org.renameme.search.aggregations.bucket.adjacency.AdjacencyMatrixAggregationBuilder;
import org.renameme.search.aggregations.bucket.adjacency.ParsedAdjacencyMatrix;
import org.renameme.search.aggregations.bucket.composite.CompositeAggregationBuilder;
import org.renameme.search.aggregations.bucket.composite.ParsedComposite;
...
import org.renameme.search.aggregations.bucket.filter.ParsedFilter;
import org.renameme.search.aggregations.bucket.filter.ParsedFilters;
...
import org.renameme.search.aggregations.bucket.geogrid.ParsedGeoHashGrid;
import org.renameme.search.aggregations.bucket.geogrid.ParsedGeoTileGrid;
import org.renameme.search.aggregations.bucket.global.GlobalAggregationBuilder;
import org.renameme.search.aggregations.bucket.global.ParsedGlobal;
...
import org.renameme.search.aggregations.bucket.histogram.ParsedAutoDateHistogram;
import org.renameme.search.aggregations.bucket.histogram.ParsedDateHistogram;
import org.renameme.search.aggregations.bucket.histogram.ParsedHistogram;
import org.renameme.search.aggregations.bucket.histogram.ParsedVariableWidthHistogram;
...
import org.renameme.search.aggregations.bucket.missing.ParsedMissing;
import org.renameme.search.aggregations.bucket.nested.NestedAggregationBuilder;
import org.renameme.search.aggregations.bucket.nested.ParsedNested;
import org.renameme.search.aggregations.bucket.nested.ParsedReverseNested;
...
import org.renameme.search.aggregations.bucket.range.ParsedBinaryRange;
import org.renameme.search.aggregations.bucket.range.ParsedDateRange;
import org.renameme.search.aggregations.bucket.range.ParsedGeoDistance;
import org.renameme.search.aggregations.bucket.range.ParsedRange;
...
import org.renameme.search.aggregations.bucket.sampler.ParsedSampler;
import org.renameme.search.aggregations.bucket.terms.LongRareTerms;
import org.renameme.search.aggregations.bucket.terms.ParsedLongRareTerms;
import org.renameme.search.aggregations.bucket.terms.ParsedSignificantLongTerms;
import org.renameme.search.aggregations.bucket.terms.ParsedSignificantStringTerms;
import org.renameme.search.aggregations.bucket.terms.ParsedStringRareTerms;
...
import org.renameme.search.aggregations.bucket.terms.ParsedDoubleTerms;
import org.renameme.search.aggregations.bucket.terms.ParsedLongTerms;
import org.renameme.search.aggregations.bucket.terms.ParsedStringTerms;
...
import org.renameme.search.aggregations.metrics.ParsedAvg;
import org.renameme.search.aggregations.metrics.ParsedCardinality;
import org.renameme.search.aggregations.metrics.ParsedExtendedStats;
import org.renameme.search.aggregations.metrics.ParsedGeoBounds;
import org.renameme.search.aggregations.metrics.ParsedGeoCentroid;
import org.renameme.search.aggregations.metrics.ParsedHDRPercentileRanks;
import org.renameme.search.aggregations.metrics.ParsedHDRPercentiles;
import org.renameme.search.aggregations.metrics.ParsedMax;
import org.renameme.search.aggregations.metrics.ParsedMedianAbsoluteDeviation;
import org.renameme.search.aggregations.metrics.ParsedMin;
import org.renameme.search.aggregations.metrics.ParsedScriptedMetric;
import org.renameme.search.aggregations.metrics.ParsedStats;
import org.renameme.search.aggregations.metrics.ParsedSum;
import org.renameme.search.aggregations.metrics.ParsedTDigestPercentileRanks;
import org.renameme.search.aggregations.metrics.ParsedTDigestPercentiles;
import org.renameme.search.aggregations.metrics.ParsedTopHits;
import org.renameme.search.aggregations.metrics.ParsedValueCount;
import org.renameme.search.aggregations.metrics.ParsedWeightedAvg;
...
import org.renameme.search.aggregations.pipeline.ParsedBucketMetricValue;
import org.renameme.search.aggregations.pipeline.ParsedDerivative;
import org.renameme.search.aggregations.pipeline.ParsedExtendedStatsBucket;
import org.renameme.search.aggregations.pipeline.ParsedPercentilesBucket;
import org.renameme.search.aggregations.pipeline.ParsedSimpleValue;
import org.renameme.search.aggregations.pipeline.ParsedStatsBucket;
 ...
 * <p>
 *
 * This class can also be sub-classed to expose additional client methods that make use of endpoints added to Renameme through plugins,
 * or to add support for custom response sections, again added to Renameme through plugins.
 * <p>
 *
 ...
 * failure scenarios. Most of the blocking calls can throw an {@link IOException} or an unchecked {@link RenamemeException} in the
 * following cases:
 *
 * <ul>
 ...
 * times out or similar cases where there is no response coming back from the Renameme server</li>
 * <li>an {@link RenamemeException} is usually thrown in case where the server returns a 4xx or 5xx error code. The high-level client
 * then tries to parse the response body error details into a generic RenamemeException and suppresses the original
...
public class RestHighLevelClient implements Closeable {
    ...
    /**
     * Creates a {@link RestHighLevelClient} given the low level {@link RestClientBuilder} that allows to build the
     * {@link RestClient} to be used to perform requests and parsers for custom response sections added to Renameme through plugins.
     */
     ...
     * a list of entries that allow to parse custom response sections added to Renameme through plugins.
     ...
     * Also subclasses can provide parsers for custom response sections added to Renameme through plugins.
     */
    ...
    /**
     * Pings the remote Renameme cluster and returns true if the ping succeeded, false otherwise
     * @param options the request options (e.g. headers), use {@link RequestOptions#DEFAULT} if nothing needs to be customized
    ...
    /**
     * Converts a {@link ResponseException} obtained from the low level REST client into an {@link RenamemeException}.
     * If a response body was returned, tries to parse it as an error returned from Renameme.
     * If no response body was returned or anything goes wrong while parsing the error, returns a new {@link RenamemeStatusException}
     * that wraps the original {@link ResponseException}. The potential exception obtained while parsing is added to the returned
     * exception as a suppressed exception. This method is guaranteed to not throw any exception eventually thrown while parsing.
     */
    protected final RenamemeStatusException parseResponseException(ResponseException responseException) {
        Response response = responseException.getResponse();
        HttpEntity entity = response.getEntity();
        RenamemeStatusException renamemeException;
        RestStatus restStatus = RestStatus.fromCode(response.getStatusLine().getStatusCode());

        if (entity == null) {
            renamemeException = new RenamemeStatusException(
                    responseException.getMessage(), restStatus, responseException);
        } else {
            try {
                renamemeException = parseEntity(entity, BytesRestResponse::errorFromXContent);
                renamemeException.addSuppressed(responseException);
            } catch (Exception e) {
                renamemeException = new RenamemeStatusException("Unable to parse response body", restStatus, responseException);
                renamemeException.addSuppressed(e);
            }
        }
        return renamemeException;
    }
    ...
    protected final <Resp> Resp parseEntity(final HttpEntity entity,
        ...
        if (entity.getContentType() == null) {
            throw new IllegalStateException("Renameme didn't return the [Content-Type] header, unable to parse response body");
        }
     ...
     * used to parse responses from Renameme. Any deprecation warnings
     * emitted there just mean that you are talking to an old version of
     * Renameme. There isn't anything you can do about the deprecation.
     */
