...
import org.renameme.action.fieldcaps.FieldCapabilities;
...
import org.renameme.common.text.Text;
import org.renameme.common.unit.Fuzziness;
...
import org.renameme.index.rankeval.EvalQueryQuality;
import org.renameme.index.rankeval.EvaluationMetric;
import org.renameme.index.rankeval.MetricDetail;
import org.renameme.index.rankeval.PrecisionAtK;
import org.renameme.index.rankeval.RankEvalRequest;
import org.renameme.index.rankeval.RankEvalResponse;
import org.renameme.index.rankeval.RankEvalSpec;
import org.renameme.index.rankeval.RatedDocument;
import org.renameme.index.rankeval.RatedRequest;
import org.renameme.index.rankeval.RatedSearchHit;
...
import org.renameme.search.aggregations.Aggregations;
import org.renameme.search.aggregations.bucket.range.Range;
import org.renameme.search.aggregations.bucket.terms.Terms;
import org.renameme.search.aggregations.bucket.terms.Terms.Bucket;
import org.renameme.search.aggregations.bucket.terms.TermsAggregationBuilder;
import org.renameme.search.aggregations.metrics.Avg;
import org.renameme.search.builder.SearchSourceBuilder;
import org.renameme.search.fetch.subphase.FetchSourceContext;
import org.renameme.search.fetch.subphase.highlight.HighlightBuilder;
import org.renameme.search.fetch.subphase.highlight.HighlightField;
import org.renameme.search.profile.ProfileResult;
import org.renameme.search.profile.ProfileShardResult;
import org.renameme.search.profile.aggregation.AggregationProfileShardResult;
import org.renameme.search.profile.query.CollectorResult;
import org.renameme.search.profile.query.QueryProfileShardResult;
...
import org.renameme.search.suggest.SuggestBuilder;
import org.renameme.search.suggest.SuggestBuilders;
...
import static org.renameme.index.query.QueryBuilders.matchQuery;
...
public class SearchDocumentationIT extends ESRestHighLevelClientTestCase {
        ...
        }
        {
            ...
            SearchResponse searchResponse = client.search(searchRequest, RequestOptions.DEFAULT);
            {
                ...
                try {
                    ...
                    assertThat(message, containsString("org.renameme.search.aggregations.bucket.terms.ParsedStringTerms"));
                    assertThat(message, containsString("org.renameme.search.aggregations.bucket.range.Range"));
                }
    ...
    public void testSearchRequestHighlighting() throws IOException {
        RestHighLevelClient client = highLevelClient();
        {
            BulkRequest request = new BulkRequest();
            request.add(new IndexRequest("posts").id("1")
                    .source(XContentType.JSON, "title", "In which order are my Renameme queries executed?", "user",
                            Arrays.asList("kimchy", "luca"), "innerObject", Collections.singletonMap("key", "value")));
            request.add(new IndexRequest("posts").id("2")
                    .source(XContentType.JSON, "title", "Current status and upcoming changes in Renameme", "user",
                            Arrays.asList("kimchy", "christoph"), "innerObject", Collections.singletonMap("key", "value")));
            request.add(new IndexRequest("posts").id("3")
                    .source(XContentType.JSON, "title", "The Future of Federated Search in Renameme", "user",
        ...
        }
        {
            ...
            searchSourceBuilder.query(QueryBuilders.boolQuery()
                    .should(matchQuery("title", "Renameme"))
            ...
            SearchResponse searchResponse = client.search(searchRequest, RequestOptions.DEFAULT);
            {
                ...
                for (SearchHit hit : hits.getHits()) {
                    ...
                    assertThat(fragments[0].string(), containsString("<em>Renameme</em>"));
    ...
    public void testSearchRequestProfiling() throws IOException {
        RestHighLevelClient client = highLevelClient();
        {
            IndexRequest request = new IndexRequest("posts").id("1")
                    .source(XContentType.JSON, "tags", "renameme", "comments", 123);
        ...
        }
        {
            ...
            searchSourceBuilder.query(QueryBuilders.termQuery("tags", "renameme"));
    ...
    public void testScroll() throws Exception {
        RestHighLevelClient client = highLevelClient();
        {
            BulkRequest request = new BulkRequest();
            request.add(new IndexRequest("posts").id("1")
                    .source(XContentType.JSON, "title", "In which order are my Renameme queries executed?"));
            request.add(new IndexRequest("posts").id("2")
                    .source(XContentType.JSON, "title", "Current status and upcoming changes in Renameme"));
            request.add(new IndexRequest("posts").id("3")
                    .source(XContentType.JSON, "title", "The Future of Federated Search in Renameme"));
            request.setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE);
            BulkResponse bulkResponse = client.bulk(request, RequestOptions.DEFAULT);
            assertSame(RestStatus.OK, bulkResponse.status());
            assertFalse(bulkResponse.hasFailures());
        }
        {
            ...
            SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
            searchSourceBuilder.query(matchQuery("title", "Renameme"));
    ...
    public void testSearchTemplateWithInlineScript() throws Exception {
        ...
        scriptParams.put("value", "renameme");
        ...
        assertEquals(
            ("{  \"size\" : \"5\",  \"query\": { \"match\" : { \"title\" : \"renameme\" } }}").replaceAll("\\s+", ""),
            source.utf8ToString()
        );
    }
    ...
    public void testMultiSearchTemplateWithInlineScript() throws Exception {
        ...
        String [] searchTerms = {"renameme", "logstash", "kibana"};
    ...
    private void indexSearchTestData() throws IOException {
        ...
        bulkRequest.add(new IndexRequest("posts").id("1")
                .source(XContentType.JSON, "id", 1, "title", "In which order are my Renameme queries executed?", "user",
                        Arrays.asList("kimchy", "luca"), "innerObject", Collections.singletonMap("key", "value")));
        bulkRequest.add(new IndexRequest("posts").id("2")
                .source(XContentType.JSON, "id", 2, "title", "Current status and upcoming changes in Renameme", "user",
                        Arrays.asList("kimchy", "christoph"), "innerObject", Collections.singletonMap("key", "value")));
        bulkRequest.add(new IndexRequest("posts").id("3")
                .source(XContentType.JSON, "id", 3, "title", "The Future of Federated Search in Renameme", "user",
                        Arrays.asList("kimchy", "tanguy"), "innerObject", Collections.singletonMap("key", "value")));
