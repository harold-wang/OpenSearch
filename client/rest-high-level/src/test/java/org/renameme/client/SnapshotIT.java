...
public class SnapshotIT extends ESRestHighLevelClientTestCase {
    ...
    public void testSnapshotGetRepositoriesNonExistent() {
        ...
        RenamemeException exception = expectThrows(RenamemeException.class, () -> execute(request,
            highLevelClient().snapshot()::getRepository, highLevelClient().snapshot()::getRepositoryAsync));

        assertThat(exception.status(), equalTo(RestStatus.NOT_FOUND));
        assertThat(exception.getMessage(), equalTo(
            "Renameme exception [type=repository_missing_exception, reason=[" + repository + "] missing]"));
    }
