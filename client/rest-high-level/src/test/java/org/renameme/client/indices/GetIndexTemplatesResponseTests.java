...
import static org.renameme.index.RandomCreateIndexGenerator.randomMappingFields;
import static org.renameme.test.AbstractXContentTestCase.xContentTester;
...
public class GetIndexTemplatesResponseTests extends ESTestCase {
    ...
    public void testParsingFromEsResponse() throws IOException {
        for (int runs = 0; runs < 20; runs++) {
            org.renameme.action.admin.indices.template.get.GetIndexTemplatesResponse esResponse =
                new org.renameme.action.admin.indices.template.get.GetIndexTemplatesResponse(new ArrayList<>());

            XContentType xContentType = randomFrom(XContentType.values());
            int numTemplates = randomIntBetween(0, 32);
            for (int i = 0; i < numTemplates; i++) {
                org.renameme.cluster.metadata.IndexTemplateMetadata.Builder esIMD =
                    new org.renameme.cluster.metadata.IndexTemplateMetadata.Builder(String.format(Locale.ROOT, "%02d ", i) +
            ...
            try (XContentParser parser = XContentHelper.createParser(NamedXContentRegistry.EMPTY,
                ...
                for (int i = 0; i < numTemplates; i++) {
                    org.renameme.cluster.metadata.IndexTemplateMetadata esIMD = esResponse.getIndexTemplates().get(i);
    ...
    static void toXContent(GetIndexTemplatesResponse response, XContentBuilder builder) throws IOException {
        ...
        List<org.renameme.cluster.metadata.IndexTemplateMetadata> serverIndexTemplates = new ArrayList<>();
        ...
        for (IndexTemplateMetadata clientITMD : clientIndexTemplates) {
            org.renameme.cluster.metadata.IndexTemplateMetadata.Builder serverTemplateBuilder =
                    org.renameme.cluster.metadata.IndexTemplateMetadata.builder(clientITMD.name());
        ...
        org.renameme.action.admin.indices.template.get.GetIndexTemplatesResponse serverResponse = new
                org.renameme.action.admin.indices.template.get.GetIndexTemplatesResponse(serverIndexTemplates);
