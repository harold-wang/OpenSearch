...
import org.renameme.action.search.SearchResponseSections;
...
import org.renameme.common.xcontent.cbor.CborXContent;
import org.renameme.common.xcontent.smile.SmileXContent;
import org.renameme.index.rankeval.DiscountedCumulativeGain;
import org.renameme.index.rankeval.EvaluationMetric;
import org.renameme.index.rankeval.ExpectedReciprocalRank;
import org.renameme.index.rankeval.MeanReciprocalRank;
import org.renameme.index.rankeval.MetricDetail;
import org.renameme.index.rankeval.PrecisionAtK;
import org.renameme.index.rankeval.RecallAtK;
...
import org.renameme.test.rest.yaml.restspec.ClientYamlSuiteRestApi;
import org.renameme.test.rest.yaml.restspec.ClientYamlSuiteRestSpec;
...
public class RestHighLevelClientTests extends ESTestCase {
    ...
    public void testParseEntity() throws IOException {
        ...
        }
        {
            ...
            assertEquals("Renameme didn't return the [Content-Type] header, unable to parse response body", ise.getMessage());
        }
        {
    ...
    public void testParseResponseException() throws IOException {
        {
            ...
            RenamemeException renamemeException = restHighLevelClient.parseResponseException(responseException);
            assertEquals(responseException.getMessage(), renamemeException.getMessage());
            assertEquals(restStatus, renamemeException.status());
            assertSame(responseException, renamemeException.getCause());
        }
        {
            RestStatus restStatus = randomFrom(RestStatus.values());
            HttpResponse httpResponse = new BasicHttpResponse(newStatusLine(restStatus));
            ...
            ResponseException responseException = new ResponseException(response);
            RenamemeException renamemeException = restHighLevelClient.parseResponseException(responseException);
            assertEquals("Renameme exception [type=exception, reason=test error message]", renamemeException.getMessage());
            assertEquals(restStatus, renamemeException.status());
            assertSame(responseException, renamemeException.getSuppressed()[0]);
        }
        {
            RestStatus restStatus = randomFrom(RestStatus.values());
            HttpResponse httpResponse = new BasicHttpResponse(newStatusLine(restStatus));
            ...
            ResponseException responseException = new ResponseException(response);
            RenamemeException renamemeException = restHighLevelClient.parseResponseException(responseException);
            assertEquals("Unable to parse response body", renamemeException.getMessage());
            assertEquals(restStatus, renamemeException.status());
            assertSame(responseException, renamemeException.getCause());
            assertThat(renamemeException.getSuppressed()[0], instanceOf(IOException.class));
        }
        {
            RestStatus restStatus = randomFrom(RestStatus.values());
            HttpResponse httpResponse = new BasicHttpResponse(newStatusLine(restStatus));
            ...
            assertEquals(restStatus, renamemeException.status());
            assertSame(responseException, renamemeException.getCause());
            assertThat(renamemeException.getSuppressed()[0], instanceOf(IllegalStateException.class));
        }
    }
    ...
    public void testPerformRequestOnResponseExceptionWithoutEntity() throws IOException {
        ...
        RenamemeException renamemeException = expectThrows(RenamemeException.class,
    ...
    public void testPerformRequestOnResponseExceptionWithBrokenEntity() throws IOException {
        ...
        assertEquals(restStatus, renamemeException.status());
        assertSame(responseException, renamemeException.getCause());
        assertThat(renamemeException.getSuppressed()[0], instanceOf(JsonParseException.class));
    }
    ...
    public void testPerformRequestOnResponseExceptionWithIgnoresErrorNoBody() throws IOException {
        ...
        assertEquals(RestStatus.NOT_FOUND, renamemeException.status());
        assertSame(responseException, renamemeException.getCause());
        assertEquals(responseException.getMessage(), renamemeException.getMessage());
    }
    ...
    public void testWrapResponseListenerOnResponseExceptionWithoutEntity() throws IOException {
        ...
        assertThat(trackingActionListener.exception.get(), instanceOf(RenamemeException.class));
        RenamemeException renamemeException = (RenamemeException) trackingActionListener.exception.get();
        assertEquals(responseException.getMessage(), renamemeException.getMessage());
        assertEquals(restStatus, renamemeException.status());
        assertSame(responseException, renamemeException.getCause());
    }
