...
import org.renameme.action.admin.cluster.node.tasks.list.TaskGroup;
...
import org.renameme.client.tasks.CancelTasksRequest;
import org.renameme.client.tasks.CancelTasksResponse;
...
public class TasksClientDocumentationIT extends ESRestHighLevelClientTestCase {
    ...
    public void testListTasks() throws IOException {
        ...
        List<RenamemeException> nodeFailures = response.getNodeFailures(); // <1>
    ...
    public void testCancelTasks() throws IOException {
        RestHighLevelClient client = highLevelClient();
        {
            ...
            CancelTasksRequest request = new org.renameme.client.tasks.CancelTasksRequest.Builder()
            ...
            CancelTasksRequest byTaskIdRequest = new org.renameme.client.tasks.CancelTasksRequest.Builder() // <1>
                .withTaskId(new org.renameme.client.tasks.TaskId("myNode",44L)) // <2>
        ...
        CancelTasksRequest request = new org.renameme.client.tasks.CancelTasksRequest.Builder().build();
        ...
        List<org.renameme.client.tasks.TaskInfo> tasks = response.getTasks(); // <1>
        // end::cancel-tasks-response-tasks

        // tag::cancel-tasks-response-calc
        Map<String, List<org.renameme.client.tasks.TaskInfo>> perNodeTasks = response.getPerNodeTasks(); // <1>
        List<org.renameme.client.tasks.TaskGroup> groups = response.getTaskGroups(); // <2>
        // end::cancel-tasks-response-calc


        // tag::cancel-tasks-response-failures
        List<org.renameme.client.tasks.RenamemeException> nodeFailures = response.getNodeFailures(); // <1>
        List<org.renameme.client.tasks.TaskOperationFailure> taskFailures = response.getTaskFailures(); // <2>
