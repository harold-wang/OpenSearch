...
import org.renameme.tasks.TaskInfo;
...
public class CancelTasksResponseTests extends AbstractResponseTestCase<CancelTasksResponseTests.ByNodeCancelTasksResponse,
    org.renameme.client.tasks.CancelTasksResponse> {
    ...
    protected CancelTasksResponseTests.ByNodeCancelTasksResponse createServerTestInstance(XContentType xContentType) {
        List<org.renameme.tasks.TaskInfo> tasks = new ArrayList<>();
        List<TaskOperationFailure> taskFailures = new ArrayList<>();
        List<RenamemeException> nodeFailures = new ArrayList<>();
        ...
        for (int i = 0; i < randomIntBetween(1, 4); i++) {
            nodeFailures.add(new RenamemeException(new RuntimeException(randomAlphaOfLength(10))));
        }

        for (int i = 0; i < 4; i++) {
            tasks.add(new org.renameme.tasks.TaskInfo(
    ...
    @Override
    protected org.renameme.client.tasks.CancelTasksResponse doParseToClientInstance(XContentParser parser) throws IOException {
        return org.renameme.client.tasks.CancelTasksResponse.fromXContent(parser);
    }

    @Override
    protected void assertInstances(ByNodeCancelTasksResponse serverTestInstance,
                                   org.renameme.client.tasks.CancelTasksResponse clientInstance) {

        // checking tasks
        List<TaskInfo> sTasks = serverTestInstance.getTasks();
        List<org.renameme.client.tasks.TaskInfo> cTasks = clientInstance.getTasks();
        Map<org.renameme.client.tasks.TaskId, org.renameme.client.tasks.TaskInfo> cTasksMap =
            cTasks.stream().collect(Collectors.toMap(org.renameme.client.tasks.TaskInfo::getTaskId,
                Function.identity()));
        for (TaskInfo ti : sTasks) {
            org.renameme.client.tasks.TaskInfo taskInfo = cTasksMap.get(
                new org.renameme.client.tasks.TaskId(ti.getTaskId().getNodeId(), ti.getTaskId().getId())
            );
        ...
        //checking failures
        List<RenamemeException> serverNodeFailures = serverTestInstance.getNodeFailures();
        List<org.renameme.client.tasks.RenamemeException> cNodeFailures = clientInstance.getNodeFailures();
        List<String> sExceptionsMessages = serverNodeFailures.stream().map(x ->
            org.renameme.client.tasks.RenamemeException.buildMessage(
                "exception", x.getMessage(), null)
        ).collect(Collectors.toList()
        );

        List<String> cExceptionsMessages = cNodeFailures.stream().map(
            org.renameme.client.tasks.RenamemeException::getMsg
        ).collect(Collectors.toList());
        ...
        List<org.renameme.client.tasks.TaskOperationFailure> cTaskFailures = clientInstance.getTaskFailures();

        Map<Long, org.renameme.client.tasks.TaskOperationFailure> cTasksFailuresMap =
            cTaskFailures.stream().collect(Collectors.toMap(
                org.renameme.client.tasks.TaskOperationFailure::getTaskId,
                Function.identity()));
        for (TaskOperationFailure tof : sTaskFailures) {
            org.renameme.client.tasks.TaskOperationFailure failure = cTasksFailuresMap.get(tof.getTaskId());
    ...
    static class ByNodeCancelTasksResponse extends CancelTasksResponse {
        ...
        ByNodeCancelTasksResponse(
            ...
            List<? extends RenamemeException> nodeFailures) {
