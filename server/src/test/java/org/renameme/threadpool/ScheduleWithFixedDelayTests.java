...
import org.renameme.threadpool.Scheduler.ReschedulingRunnable;
...
public class ScheduleWithFixedDelayTests extends ESTestCase {
    ...
    public void testThatRunnableIsRescheduled() throws Exception {
        ...
        final Runnable countingRunnable = () -> {
            if (rarely()) {
                throw new RenamemeException("sometimes we throw before counting down");
            }

            latch.countDown();

            if (randomBoolean()) {
                throw new RenamemeException("this shouldn't cause the test to fail!");
            }
        };
