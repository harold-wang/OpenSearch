...
public class InboundHandlerTests extends ESTestCase {
    ...
    public void testRequestAndResponse() throws Exception {
        ...
        if (isError) {
            ...
            transportChannel.sendResponse(new RenamemeException("boom"));
        } else {
        ...
        if (isError) {
            ...
            assertThat(exceptionCaptor.get().getCause(), instanceOf(RenamemeException.class));
    ...
    public void testSendsErrorResponseToHandshakeFromCompatibleVersion() throws Exception {
        ...
        requestHeader.headers = Tuple.tuple(org.renameme.common.collect.Map.of(), org.renameme.common.collect.Map.of());
        requestHeader.features = org.renameme.common.collect.Set.of();
        handler.inboundMessage(channel, requestMessage);
        ...
        return new InboundMessage(requestHeader, ReleasableBytesReference.wrap(BytesArray.EMPTY), () -> { }) {
            ...
            public StreamInput openOrGetStreamInput() {
                final StreamInput streamInput = new InputStreamStreamInput(new InputStream() {
                    @Override
                    public int read() {
                        throw new RenamemeException("unreadable handshake");
                    }
                });
