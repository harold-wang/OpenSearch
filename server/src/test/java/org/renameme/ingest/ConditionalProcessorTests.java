...
import org.renameme.script.IngestConditionalScript;
import org.renameme.ingest.RandomDocumentPicks;
...
import org.renameme.script.StoredScriptSource;
...
public class ConditionalProcessorTests extends ESTestCase {
    ...
    public void testPrecompiledError() {
        ScriptService scriptService = MockScriptService.singleContext(IngestConditionalScript.CONTEXT, code -> {
            throw new ScriptException("bad script", new ParseException("error", 0),
                org.renameme.common.collect.List.of(), "", "lang", null);
        }, org.renameme.common.collect.Map.of());
        Script script = new Script(ScriptType.INLINE, "lang", "foo", org.renameme.common.collect.Map.of());
    ...
    public void testRuntimeCompileError() {
        ...
        storedScripts.put("foo", new StoredScriptSource("lang", "", org.renameme.common.collect.Map.of()));
        ScriptService scriptService = MockScriptService.singleContext(IngestConditionalScript.CONTEXT, code -> {
            if (fail.get()) {
                throw new ScriptException("bad script", new ParseException("error", 0),
                    org.renameme.common.collect.List.of(), "", "lang", null);
        ...
        Script script = new Script(ScriptType.STORED, null, "foo", org.renameme.common.collect.Map.of());
        ...
        storedScripts.put("foo", new StoredScriptSource("lang", "changed", org.renameme.common.collect.Map.of()));
        IngestDocument ingestDoc = new IngestDocument(org.renameme.common.collect.Map.of(),
            org.renameme.common.collect.Map.of());
