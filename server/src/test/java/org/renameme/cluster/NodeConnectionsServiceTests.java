...
import org.renameme.transport.TransportMessageListener;
...
import org.renameme.transport.TransportStats;
...
import static org.renameme.cluster.NodeConnectionsService.CLUSTER_NODE_RECONNECT_INTERVAL_SETTING;
...
import static org.renameme.common.util.concurrent.ConcurrentCollections.newConcurrentMap;
...
public class NodeConnectionsServiceTests extends ESTestCase {
    ...
    public void testOnlyBlocksOnConnectionsToNewNodes() throws Exception {
        ...
        try {
            ...
            expectThrows(RenamemeTimeoutException.class, () -> future3.actionGet(timeValueMillis(scaledRandomIntBetween(1, 1000))));
            ...
            expectThrows(RenamemeTimeoutException.class, () -> future6.actionGet(timeValueMillis(scaledRandomIntBetween(1, 1000))));
    ...
    @TestLogging(reason="testing that DEBUG-level logging is reasonable", value="org.renameme.cluster.NodeConnectionsService:DEBUG")
    public void testDebugLogging() throws IllegalAccessException {
        ...
        try {
            appender.start();
            Loggers.addAppender(LogManager.getLogger("org.renameme.cluster.NodeConnectionsService"), appender);
            for (DiscoveryNode targetNode : targetNodes) {
                if (disconnectedNodes.contains(targetNode)) {
                    appender.addExpectation(new MockLogAppender.SeenEventExpectation("connecting to " + targetNode,
                        "org.renameme.cluster.NodeConnectionsService", Level.DEBUG,
            ...
            Loggers.removeAppender(LogManager.getLogger("org.renameme.cluster.NodeConnectionsService"), appender);
            appender.stop();
        }        for (DiscoveryNode disconnectedNode : disconnectedNodes) {
            transportService.disconnectFromNode(disconnectedNode);
        }
