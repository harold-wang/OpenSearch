...
public class MasterServiceTests extends ESTestCase {
   ...
   */
    ...
    @TestLogging(value = "org.renameme.cluster.service:TRACE", reason = "to ensure that we log cluster state events on TRACE level")
    ...
    @TestLogging(value = "org.renameme.cluster.service:WARN", reason = "to ensure that we log cluster state events on WARN level")
        ...
        try (MasterService masterService = new MasterService(Settings.builder()
            ...
            masterService.setClusterStatePublisher((event, publishListener, ackListener) -> {
                ...
                if (event.source().contains("test6")) {
                    relativeTimeInMillis += MasterService.MASTER_SERVICE_SLOW_TASK_LOGGING_THRESHOLD_SETTING.get(Settings.EMPTY).millis()
                        + randomLongBetween(1, 1000000);
                    throw new RenamemeException("simulated error during slow publication which should trigger logging");
                }
                clusterStateRef.set(event.state());
                publishListener.onResponse(null);
            });
