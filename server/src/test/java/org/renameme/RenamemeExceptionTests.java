...
import org.renameme.action.NoShardAvailableActionException;
...
import org.renameme.action.support.broadcast.BroadcastShardOperationFailedException;
...
import org.renameme.index.shard.IndexShardRecoveringException;
import org.renameme.index.shard.ShardId;
import org.renameme.node.NodeClosedException;
import org.renameme.repositories.RepositoryException;
import org.renameme.rest.RestStatus;
import org.renameme.script.ScriptException;
...
import org.renameme.transport.RemoteTransportException;
...
import static org.renameme.test.hamcrest.RenamemeAssertions.assertToXContentEquivalent;
...
public class RenamemeExceptionTests extends ESTestCase {

    public void testStatus() {
        RenamemeException exception = new RenamemeException("test");
        assertThat(exception.status(), equalTo(RestStatus.INTERNAL_SERVER_ERROR));

        exception = new RenamemeException("test", new RuntimeException());
        assertThat(exception.status(), equalTo(RestStatus.INTERNAL_SERVER_ERROR));

        exception = new RenamemeException("test", new ResourceNotFoundException("test"));
        assertThat(exception.status(), equalTo(RestStatus.INTERNAL_SERVER_ERROR));
    ...
    public void testGuessRootCause() {
        {
            RenamemeException exception = new RenamemeException("foo", new RenamemeException("bar",
                    new IndexNotFoundException("foo", new RuntimeException("foobar"))));
            RenamemeException[] rootCauses = exception.guessRootCauses();
            assertEquals(rootCauses.length, 1);
            assertEquals(RenamemeException.getExceptionName(rootCauses[0]), "index_not_found_exception");
            ...
            if (randomBoolean()) {
                ...
                rootCauses = RenamemeException.guessRootCauses(randomBoolean() ? new RemoteTransportException("remoteboom", ex) : ex);
            }
            assertEquals("parsing_exception", RenamemeException.getExceptionName(rootCauses[0]));
            assertEquals("foobar", rootCauses[0].getMessage());

            RenamemeException oneLevel = new RenamemeException("foo", new RuntimeException("foobar"));
            rootCauses = oneLevel.guessRootCauses();
            assertEquals("exception", RenamemeException.getExceptionName(rootCauses[0]));
        ...
        }
        {
            ...
            final RenamemeException[] rootCauses = ex.guessRootCauses();
            assertEquals(rootCauses.length, 2);
            assertEquals(RenamemeException.getExceptionName(rootCauses[0]), "parsing_exception");
            ...
            assertEquals("query_shard_exception", RenamemeException.getExceptionName(rootCauses[1]));
            assertEquals("foo1", rootCauses[1].getIndex().getName());
            assertEquals("foobar", rootCauses[1].getMessage());
        }

        {
            final RenamemeException[] foobars = RenamemeException.guessRootCauses(new IllegalArgumentException("foobar"));
            assertEquals(foobars.length, 1);
            assertThat(foobars[0], instanceOf(RenamemeException.class));
        ...
        {
            final RenamemeException[] foobars = RenamemeException.guessRootCauses(
...

        {
            ...
            final RenamemeException[] causes = RenamemeException.guessRootCauses(outer);
            assertEquals(causes.length, 1);
            assertThat(causes[0], instanceOf(RenamemeException.class));
            assertEquals("inner", causes[0].getMessage());
            assertEquals("x_content_parse_exception", causes[0].getExceptionName());
        }

        {
            RenamemeException inner = new RenamemeException("inner");
            XContentParseException outer = new XContentParseException(null, "outer", inner);
            final RenamemeException[] causes = RenamemeException.guessRootCauses(outer);
            assertEquals(causes.length, 1);
            assertThat(causes[0], instanceOf(RenamemeException.class));
    ...
    public void testToString() {
        ...
        assertEquals("RenamemeException[foo]; nested: RenamemeException[bar]; nested: IllegalArgumentException" +
                "[index is closed]; nested: RuntimeException[foobar];", exception.toString());
    }

    public void testToXContent() throws IOException {
        {
            RenamemeException e = new RenamemeException("test");
            assertExceptionAsJson(e, "{\"type\":\"exception\",\"reason\":\"test\"}");
        }
        {
            RenamemeException e = new IndexShardRecoveringException(new ShardId("_test", "_0", 5));
        ...
        {
            RenamemeException e = new BroadcastShardOperationFailedException(new ShardId("_index", "_uuid", 12), "foo",
                    new IllegalStateException("bar"));
            assertExceptionAsJson(e, "{\"type\":\"illegal_state_exception\",\"reason\":\"bar\"}");
        }
        {
            RenamemeException e = new RenamemeException(new IllegalArgumentException("foo"));
        ...
        {
            RenamemeException e = new SearchParseException(SHARD_TARGET, "foo", new XContentLocation(1,0));
            assertExceptionAsJson(e, "{\"type\":\"search_parse_exception\",\"reason\":\"foo\",\"line\":1,\"col\":0}");
        }
        {
            RenamemeException ex = new RenamemeException("foo",
                    new RenamemeException("bar", new IllegalArgumentException("index is closed", new RuntimeException("foobar"))));
        ...
        {
            RenamemeException e = new RenamemeException("foo", new IllegalStateException("bar"));
            ...
            ToXContent.Params params = new ToXContent.MapParams(
                    Collections.singletonMap(RenamemeException.REST_EXCEPTION_SKIP_STACK_TRACE, "false"));
            String actual;
    ...
    public void testGenerateThrowableToXContent() throws IOException {
        ...
        { // test equivalence
            RenamemeException ex =  new RemoteTransportException("foobar", new FileNotFoundException("foo not found"));
            String toXContentString = Strings.toString(ex);
            String throwableString = Strings.toString((builder, params) -> {
                RenamemeException.generateThrowableXContent(builder, params, ex);
                return builder;
            });
    ...
    public void testToXContentWithHeadersAndMetadata() throws IOException {
        RenamemeException e = new RenamemeException("foo",
            new RenamemeException("bar",
                new RenamemeException("baz",
        ...
        RenamemeException parsed;
        try (XContentParser parser = createParser(XContentType.JSON.xContent(), expectedJson)) {
            assertEquals(XContentParser.Token.START_OBJECT, parser.nextToken());
            parsed = RenamemeException.fromXContent(parser);
            assertEquals(XContentParser.Token.END_OBJECT, parser.currentToken());
            assertNull(parser.nextToken());
        }

        assertNotNull(parsed);
        assertEquals(parsed.getMessage(), "Renameme exception [type=exception, reason=foo]");
        ...
        RenamemeException cause = (RenamemeException) parsed.getCause();
        assertEquals(cause.getMessage(), "Renameme exception [type=exception, reason=bar]");

        cause = (RenamemeException) cause.getCause();
        assertEquals(cause.getMessage(), "Renameme exception [type=exception, reason=baz]");

        cause = (RenamemeException) cause.getCause();
        assertEquals(cause.getMessage(),
                "Renameme exception [type=cluster_block_exception, reason=blocked by: [SERVICE_UNAVAILABLE/2/no master];]");
    }
    ...
    public void testFromXContentWithIgnoredMetadataAndHeaders() throws IOException {
        ...
        RenamemeException parsedException;
        try (XContentParser parser = createParser(xContent, originalBytes)) {
            assertEquals(XContentParser.Token.START_OBJECT, parser.nextToken());
            parsedException = RenamemeException.fromXContent(parser);
            assertEquals(XContentParser.Token.END_OBJECT, parser.currentToken());
            assertNull(parser.nextToken());
        }
    ...
    public void testThrowableToAndFromXContent() throws IOException {
        ...
        final Tuple<Throwable, RenamemeException> exceptions = randomExceptions();
        final Throwable throwable = exceptions.v1();
        final RenamemeException expected = exceptions.v2();
        int suppressedCount = randomBoolean() ? 0 : between(1, 5);
        for (int i = 0; i < suppressedCount; i++) {
            final Tuple<Throwable, RenamemeException> suppressed = randomExceptions();
        ...
        BytesReference throwableBytes = toShuffledXContent((builder, params) -> {
            RenamemeException.generateThrowableXContent(builder, params, throwable);
            return builder;
        ...
        if (suppressedCount > 0) {
            XContentBuilder builder = XContentBuilder.builder(xContent);
            builder.startObject();
            RenamemeException.generateThrowableXContent(builder, ToXContent.EMPTY_PARAMS, throwable);
            builder.endObject();
    ...
    public void testUnknownFailureToAndFromXContent() throws IOException {
        ...
        BytesReference failureBytes = toShuffledXContent((builder, params) -> {
            ...
            RenamemeException.generateFailureXContent(builder, params, null, randomBoolean());
            return builder;
        }, xContent.type(), ToXContent.EMPTY_PARAMS, randomBoolean());

        RenamemeException parsedFailure;
        try (XContentParser parser = createParser(xContent, failureBytes)) {
            assertEquals(XContentParser.Token.START_OBJECT, parser.nextToken());
            assertEquals(XContentParser.Token.FIELD_NAME, parser.nextToken());
            parsedFailure = RenamemeException.failureFromXContent(parser);
    ...
    public void testFailureToAndFromXContentWithNoDetails() throws IOException {
        ...
        BytesReference failureBytes = toShuffledXContent((builder, params) -> {
            RenamemeException.generateFailureXContent(builder, params, failure, false);
            return builder;
        }, xContent.type(), ToXContent.EMPTY_PARAMS, randomBoolean());
        ...
        String reason;
        if (failure instanceof RenamemeException) {
            reason = failure.getClass().getSimpleName() + "[" + failure.getMessage() + "]";
        } else {
            reason = "No RenamemeException found";
        }
        assertEquals(RenamemeException.buildMessage("exception", reason, null), parsedFailure.getMessage());
        assertEquals(0, parsedFailure.getHeaders().size());
        assertEquals(0, parsedFailure.getMetadata().size());
    ...
    public void testFailureToAndFromXContentWithDetails() throws IOException {
        ...
        RenamemeException expected;
        RenamemeException expectedCause;
        RenamemeException suppressed;

        switch (randomIntBetween(0, 6)) {
            case 0: // Simple renameme exception without cause
                failure = new NoNodeAvailableException("A");

                expected = new RenamemeException("Elasticsearch exception [type=no_node_available_exception, reason=A]");
                expected.addSuppressed(new RenamemeException("Elasticsearch exception [type=no_node_available_exception, reason=A]"));
                break;

            case 1: // Simple renameme exception with headers (other metadata of type number are not parsed)
                failure = new ParsingException(3, 2, "B", null);
                ((RenamemeException) failure).addHeader("header_name", "0", "1");
                expected = new RenamemeException("Elasticsearch exception [type=parsing_exception, reason=B]");
                expected.addHeader("header_name", "0", "1");
                suppressed = new RenamemeException("Elasticsearch exception [type=parsing_exception, reason=B]");
            ...
            case 2: // Elasticsearch exception with a cause, headers and parsable metadata
                ...
                ((RenamemeException) failure).addHeader("script_name", "my_script");

                expectedCause = new RenamemeException("Elasticsearch exception [type=null_pointer_exception, reason=var is null]");
                expected = new RenamemeException("Elasticsearch exception [type=script_exception, reason=C]", expectedCause);
                ...
                suppressed = new RenamemeException("Elasticsearch exception [type=script_exception, reason=C]");
            ...
            case 3: // JDK exception without cause
                ...
                expected = new RenamemeException("Elasticsearch exception [type=illegal_state_exception, reason=D]");
                suppressed = new RenamemeException("Elasticsearch exception [type=illegal_state_exception, reason=D]");
                expected.addSuppressed(suppressed);
                break;
            ...
            case 4: // JDK exception with cause
                ...
                expectedCause = new RenamemeException("Elasticsearch exception [type=routing_missing_exception, " +
                ...
                expected = new RenamemeException("Elasticsearch exception [type=runtime_exception, reason=E]", expectedCause);
                suppressed = new RenamemeException("Elasticsearch exception [type=runtime_exception, reason=E]");
                expected.addSuppressed(suppressed);
                break;
            ...
            case 5: // Wrapped exception with cause
                ...
                expected = new RenamemeException("Elasticsearch exception [type=file_already_exists_exception, reason=File exists]");
                suppressed = new RenamemeException("Elasticsearch exception [type=file_already_exists_exception, reason=File exists]");
                expected.addSuppressed(suppressed);
                break;
            ...
            case 6: // SearchPhaseExecutionException with cause and multiple failures
                ...
                expectedCause = new RenamemeException("Elasticsearch exception [type=node_closed_exception, " +
                        "reason=node closed " + node + "]");
                expectedCause = new RenamemeException("Elasticsearch exception [type=no_shard_available_action_exception, " +
                ...
                expected = new RenamemeException("Elasticsearch exception [type=search_phase_execution_exception, " +
                        "reason=G]", expectedCause);
                expected.addMetadata("es.phase", "phase_g");

                expected.addSuppressed(new RenamemeException("Elasticsearch exception [type=parsing_exception, reason=Parsing g]"));
                expected.addSuppressed(new RenamemeException("Elasticsearch exception [type=repository_exception, " +
                        "reason=[repository_g] Repo]"));
                expected.addSuppressed(new RenamemeException("Elasticsearch exception [type=search_context_missing_exception, " +
        ...
        BytesReference failureBytes = toShuffledXContent((builder, params) -> {
            RenamemeException.generateFailureXContent(builder, params, finalFailure, true);
            return builder;
        }, xContent.type(), ToXContent.EMPTY_PARAMS, randomBoolean());
    ...
    private static void assertExceptionAsJson(Exception e, String expectedJson) throws IOException {
        assertToXContentAsJson((builder, params) -> {
            RenamemeException.generateThrowableXContent(builder, params, e);
            return builder;
        }, expectedJson);
    }

    public static void assertDeepEquals(RenamemeException expected, RenamemeException actual) {
        do {
            if (expected == null) {
                assertNull(actual);
            } else {
            ...
            if (expectedSuppressed == null) {
                ...
                for (int i = 0; i < expectedSuppressed.length; i++) {
                    assertDeepEquals((RenamemeException) expectedSuppressed[i], (RenamemeException) actualSuppressed[i]);
                }
            }

            expected = (RenamemeException) expected.getCause();
            actual = (RenamemeException) actual.getCause();
            if (expected == null) {
                assertNull(actual);
            }
        } while (expected != null);
    ...
    public static Tuple<Throwable, RenamemeException> randomExceptions() {
        Throwable actual;
        RenamemeException expected;
        ...
        switch (type) {
            case 0:
                ...
                expected = new RenamemeException("Elasticsearch exception [type=cluster_block_exception, " +
                        "reason=blocked by: [SERVICE_UNAVAILABLE/2/no master];]");
                break;
            case 1: // Simple renameme exception with headers (other metadata of type number are not parsed)
                actual = new ParsingException(3, 2, "Unknown identifier", null);
                expected = new RenamemeException("Elasticsearch exception [type=parsing_exception, reason=Unknown identifier]");
                break;
            case 2:
                actual = new SearchParseException(SHARD_TARGET, "Parse failure", new XContentLocation(12, 98));
                expected = new RenamemeException("Elasticsearch exception [type=search_parse_exception, reason=Parse failure]");
                break;
            case 3:
                actual = new IllegalArgumentException("Closed resource", new RuntimeException("Resource"));
                expected = new RenamemeException("Elasticsearch exception [type=illegal_argument_exception, reason=Closed resource]",
                                new RenamemeException("Elasticsearch exception [type=runtime_exception, reason=Resource]"));
                break;
            case 4:
                ...
                break;
            case 5:
                actual = new RenamemeException("Parsing failed",
                            new ParsingException(9, 42, "Wrong state",
                                new NullPointerException("Unexpected null value")));

                RenamemeException expectedCause = new RenamemeException("Elasticsearch exception [type=parsing_exception, " +
                        "reason=Wrong state]", new RenamemeException("Elasticsearch exception [type=null_pointer_exception, " +
                        "reason=Unexpected null value]"));
                expected = new RenamemeException("Elasticsearch exception [type=exception, reason=Parsing failed]", expectedCause);
                break;
            default:
                throw new UnsupportedOperationException("No randomized exceptions generated for type [" + type + "]");
        }

        if (actual instanceof RenamemeException) {
            RenamemeException actualException = (RenamemeException) actual;
            if (randomBoolean()) {
