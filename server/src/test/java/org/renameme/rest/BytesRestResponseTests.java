...
import static org.renameme.RenamemeExceptionTests.assertDeepEquals;
...
public class BytesRestResponseTests extends ESTestCase {
    ...
    public void testSimpleExceptionMessage() throws Exception {
        ...
        Exception t = new RenamemeException("an error occurred reading data", new FileNotFoundException("/foo/bar"));
        BytesRestResponse response = new BytesRestResponse(channel, t);
        String text = response.content().utf8ToString();
        assertThat(text, containsString("RenamemeException[an error occurred reading data]"));
    ...
    public void testNonRenamemeExceptionIsNotShownAsSimpleMessage() throws Exception {
        RestRequest request = new FakeRestRequest();
        RestChannel channel = new SimpleExceptionRestChannel(request);
        ...
        assertThat(text, not(containsString("error_trace")));
        assertThat(text, containsString("\"error\":\"No RenamemeException found\""));
    }
    ...
    public void testGuessRootCause() throws IOException {
        ...
        RestChannel channel = new DetailedExceptionRestChannel(request);
        {
            Exception e = new RenamemeException("an error occurred reading data", new FileNotFoundException("/foo/bar"));
    ...
    public void testResponseWhenInternalServerError() throws IOException {
        ...
        final RestChannel channel = new DetailedExceptionRestChannel(request);
        final BytesRestResponse response = new BytesRestResponse(channel, new RenamemeException("simulated"));
        assertNotNull(response.content());
        final String content = response.content().utf8ToString();
    ...
    public void testErrorToAndFromXContent() throws IOException {
        ...
        switch (randomIntBetween(0, 5)) {
            case 0:
                original = new RenamemeException("RenamemeException without cause");
                if (detailed) {
                    addHeadersOrMetadata = randomBoolean();
                    reason = "RenamemeException without cause";
                } else {
                    reason = "RenamemeException[RenamemeException without cause]";
                }
                break;
            case 1:
                original = new RenamemeException("RenamemeException with a cause", new FileNotFoundException("missing"));
                if (detailed) {
                    addHeadersOrMetadata = randomBoolean();
                    type = "exception";
                    reason = "RenamemeException with a cause";
                    cause = new RenamemeException("Elasticsearch exception [type=file_not_found_exception, reason=missing]");
                } else {
                    reason = "RenamemeException[RenamemeException with a cause]";
                }
                break;
            case 2:
                original = new ResourceNotFoundException("RenamemeException with custom status");
                status = RestStatus.NOT_FOUND;
                if (detailed) {
                    addHeadersOrMetadata = randomBoolean();
                    type = "resource_not_found_exception";
                    reason = "RenamemeException with custom status";
                } else {
                    reason = "ResourceNotFoundException[RenamemeException with custom status]";
                }
                break;
            case 3:
                TransportAddress address = buildNewFakeTransportAddress();
                original = new RemoteTransportException("remote", address, "action",
                        new ResourceAlreadyExistsException("RenamemeWrapperException with a cause that has a custom status"));
                status = RestStatus.BAD_REQUEST;
                if (detailed) {
                    type = "resource_already_exists_exception";
                    reason = "RenamemeWrapperException with a cause that has a custom status";
                } else {
                    reason = "RemoteTransportException[[remote][" + address.toString() + "][action]]";
                }
                break;
            case 4:
                original = new RemoteTransportException("RenamemeWrapperException with a cause that has a special treatment",
                ...
                if (detailed) {
                    ...
                    reason = "RemoteTransportException[[RenamemeWrapperException with a cause that has a special treatment]]";
                }
                break;
            case 5:
                status = randomFrom(RestStatus.values());
                original = new RenamemeStatusException("RenamemeStatusException with random status", status);
                if (detailed) {
                    addHeadersOrMetadata = randomBoolean();
                    type = "status_exception";
                    reason = "RenamemeStatusException with random status";
                } else {
                    reason = "RenamemeStatusException[RenamemeStatusException with random status]";
                }
                break;
            default:
        ...
        RenamemeStatusException expected = new RenamemeStatusException(message, status, cause);

        if (addHeadersOrMetadata) {
            RenamemeException originalException = ((RenamemeException) original);
            if (randomBoolean()) {
    ...
    public void testNoErrorFromXContent() throws IOException {
        ...
        assertEquals("Failed to parse renameme status exception: no exception was found", e.getMessage());
    }

    public static class WithHeadersException extends RenamemeException {
