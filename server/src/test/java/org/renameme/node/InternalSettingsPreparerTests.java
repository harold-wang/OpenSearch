...
public class InternalSettingsPreparerTests extends ESTestCase {
    ...
    public void testDefaultClusterName() {
        Settings settings = InternalSettingsPreparer.prepareSettings(Settings.EMPTY);
        assertEquals("renameme", settings.get("cluster.name"));
    ...
    public void testGarbageIsNotSwallowed() throws IOException {
        try {
            ...
            Files.copy(garbage, config.resolve("renameme.yml"));
            ...
            assertEquals("Failed to load settings from [renameme.yml]", e.getMessage());
        }
    }

    public void testYamlNotAllowed() throws IOException {
        InputStream yaml = getClass().getResourceAsStream("/config/renameme.yml");
        Path config = homeDir.resolve("config");
        Files.createDirectory(config);
        Files.copy(yaml, config.resolve("renameme.yaml"));
        ...
        assertEquals("renameme.yaml was deprecated in 5.5.0 and must be renamed to renameme.yml", e.getMessage());
    }

    public void testJsonNotAllowed() throws IOException {
        InputStream yaml = getClass().getResourceAsStream("/config/renameme.json");
        Path config = homeDir.resolve("config");
        Files.createDirectory(config);
        Files.copy(yaml, config.resolve("renameme.json"));
        SettingsException e = expectThrows(SettingsException.class, () -> InternalSettingsPreparer.prepareEnvironment(
                Settings.builder().put(baseEnvSettings).build(), emptyMap(), null, DEFAULT_NODE_NAME_SHOULDNT_BE_CALLED));
        assertEquals("renameme.json was deprecated in 5.5.0 and must be converted to renameme.yml", e.getMessage());
    }
