...
import org.renameme.action.bulk.TransportBulkActionTookTests.Resolver;
...
import org.renameme.cluster.metadata.IndexAbstraction.Index;
...
import static org.renameme.action.bulk.TransportBulkAction.prohibitCustomRoutingOnDataStream;
import static org.renameme.cluster.metadata.MetadataCreateDataStreamServiceTests.createDataStream;
...
public class TransportBulkActionTests extends ESTestCase {
    ...
    public void testIncludesSystem() {
        ...
        indicesLookup.put(".bar",
            new Index(IndexMetadata.builder(".bar").settings(settings).system(true).numberOfShards(1).numberOfReplicas(0).build()));
        SystemIndices systemIndices = new SystemIndices(org.renameme.common.collect.Map.of("plugin",
            org.renameme.common.collect.List.of(new SystemIndexDescriptor(".test", ""))));
        List<String> onlySystem = org.renameme.common.collect.List.of(".foo", ".bar");
        assertTrue(bulkAction.includesSystem(buildBulkRequest(onlySystem), indicesLookup, systemIndices));

        onlySystem = org.renameme.common.collect.List.of(".foo", ".bar", ".test");
        assertTrue(bulkAction.includesSystem(buildBulkRequest(onlySystem), indicesLookup, systemIndices));

        List<String> nonSystem = org.renameme.common.collect.List.of("foo", "bar");
        assertFalse(bulkAction.includesSystem(buildBulkRequest(nonSystem), indicesLookup, systemIndices));

        List<String> mixed = org.renameme.common.collect.List.of(".foo", ".test", "other");
