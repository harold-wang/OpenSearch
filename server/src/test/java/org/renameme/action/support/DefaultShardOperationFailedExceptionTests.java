...
public class DefaultShardOperationFailedExceptionTests extends ESTestCase {
    ...
    public void testToString() {
        {
            DefaultShardOperationFailedException exception = new DefaultShardOperationFailedException(
                new RenamemeException("foo", new IllegalArgumentException("bar", new RuntimeException("baz"))));
            assertEquals("[null][-1] failed, reason [RenamemeException[foo]; nested: " +
                "IllegalArgumentException[bar]; nested: RuntimeException[baz]; ]", exception.toString());
        }
        {
            RenamemeException RenamemeException = new RenamemeException("foo");
            RenamemeException.setIndex(new Index("index1", "_na_"));
            RenamemeException.setShard(new ShardId("index1", "_na_", 1));
            DefaultShardOperationFailedException exception = new DefaultShardOperationFailedException(RenamemeException);
            assertEquals("[index1][1] failed, reason [RenamemeException[foo]]", exception.toString());
        }
        {
    ...
    public void testToXContent() throws IOException {
        {
            DefaultShardOperationFailedException exception = new DefaultShardOperationFailedException(new RenamemeException("foo"));
            assertEquals("{\"shard\":-1,\"index\":null,\"status\":\"INTERNAL_SERVER_ERROR\"," +
                "\"reason\":{\"type\":\"exception\",\"reason\":\"foo\"}}", Strings.toString(exception));
        }
        {
            DefaultShardOperationFailedException exception = new DefaultShardOperationFailedException(
                new RenamemeException("foo", new IllegalArgumentException("bar")));
    ...
    private static DefaultShardOperationFailedException randomInstance() {
        ...
        if (cause instanceof RenamemeException) {
            return new DefaultShardOperationFailedException((RenamemeException) cause);
        } else {
