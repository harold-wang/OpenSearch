...
package org.renameme.action.support.replication;
...
import org.renameme.index.shard.IndexShardClosedException;
...
import org.renameme.indices.IndexClosedException;
import org.renameme.indices.IndicesService;
import org.renameme.indices.breaker.NoneCircuitBreakerService;
import org.renameme.indices.cluster.ClusterStateChanges;
...
import org.renameme.transport.TransportResponse;
...
import static org.renameme.action.support.replication.ClusterStateCreationUtils.stateWithActivePrimary;
import static org.renameme.cluster.metadata.IndexMetadata.SETTING_WAIT_FOR_ACTIVE_SHARDS;
    ...
    */
    ...
    public void testStalePrimaryShardOnReroute() {
        ...
        if (timeout) {
            ...
            assertPhase(task, "waiting_on_primary");
            transport.handleRemoteError(capturedRequests[0].requestId, randomRetryPrimaryException(shardId));
            assertListenerThrows("must throw index not found exception", listener, RenamemeException.class);
            assertPhase(task, "failed");
        } else {
            assertThat(listener.isDone(), equalTo(false));
    ...
    public void testPrimaryReference() {
        ...
        shard.runUnderPrimaryPermit(() ->
            primary.perform(request, ActionTestUtils.assertNoFailureListener(r -> {
                final RenamemeException exception = new RenamemeException("testing");
    ...
    public void testReplicaProxy() throws InterruptedException, ExecutionException {
        ...
        if (randomBoolean()) {
            ...
            transport.handleRemoteError(captures[0].requestId, new RenamemeException("simulated"));
            assertTrue(listener.isDone());
            assertListenerThrows("listener should reflect remote error", listener, RenamemeException.class);
        } else {
        ...
        proxy.failShardIfNeeded(replica, primaryTerm, "test", new RenamemeException("simulated"),
    ...
    public void testCounterOnPrimary() throws Exception {
        ...
        new TestAction(Settings.EMPTY, "internal:testAction2", transportService, clusterService, shardStateAction, threadPool) {
            ...
            protected void shardOperationOnPrimary(Request shardRequest, IndexShard primary,
                ...
                if (throwExceptionOnRun) {
                    throw new RenamemeException("simulated exception, during shardOperationOnPrimary");
                } else if (respondWithError) {
                    listener.onFailure(new RenamemeException("simulated exception, as a response"));
                } else {
        ...
        try {
            ...
            if (throwExceptionOnRun || respondWithError) {
                ...
                assertThat(cause, instanceOf(RenamemeException.class));
        ...
        TestAction action = new TestAction(Settings.EMPTY, "internal:testActionWithExceptions", transportService, clusterService,
            ...
            protected void shardOperationOnReplica(Request shardRequest, IndexShard replica, ActionListener<ReplicaResult> listener) {
                ActionListener.completeWith(listener, () -> {
                    ...
                    if (throwException) {
                        throw new RenamemeException("simulated");
                    }
    ...
    /**
     * test throwing a {@link org.renameme.action.support.replication.TransportReplicationAction.RetryOnReplicaException}
     * causes a retry
     */
