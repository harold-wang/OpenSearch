...
public class RoundingDuelTests extends ESTestCase  {
    ...
    public void testSerialization() throws Exception {
        org.renameme.common.Rounding.DateTimeUnit randomDateTimeUnit =
            randomFrom(org.renameme.common.Rounding.DateTimeUnit.values());
        org.renameme.common.Rounding rounding;
        boolean oldNextRoundingValueWorks;
        if (randomBoolean()) {
            rounding = org.renameme.common.Rounding.builder(randomDateTimeUnit).timeZone(ZoneOffset.UTC).build();
            oldNextRoundingValueWorks = true;
        } else {
            rounding = org.renameme.common.Rounding.builder(timeValue()).timeZone(ZoneOffset.UTC).build();
        ...
        org.renameme.common.Rounding roundingJavaTime =
            org.renameme.common.Rounding.read(output.bytes().streamInput());
    ...
    public void testDuellingImplementations() {
        org.renameme.common.Rounding.DateTimeUnit randomDateTimeUnit =
            randomFrom(org.renameme.common.Rounding.DateTimeUnit.values());
        org.renameme.common.Rounding.Prepared rounding;
        Rounding roundingJoda;

        if (randomBoolean()) {
            rounding = org.renameme.common.Rounding.builder(randomDateTimeUnit).timeZone(ZoneOffset.UTC).build().prepareForUnknown();
            ...
            rounding = org.renameme.common.Rounding.builder(interval).timeZone(ZoneOffset.UTC).build().prepareForUnknown();
