...
public class XContentTypeTests extends ESTestCase {
    ...
    public void testVersionedMediaType() throws Exception {
        assertThat(XContentType.fromMediaTypeOrFormat("application/vnd.renameme+json;compatible-with=7"),
            equalTo(XContentType.JSON));
        assertThat(XContentType.fromMediaTypeOrFormat("application/vnd.renameme+yaml;compatible-with=7"),
            equalTo(XContentType.YAML));
        assertThat(XContentType.fromMediaTypeOrFormat("application/vnd.renameme+cbor;compatible-with=7"),
            equalTo(XContentType.CBOR));
        assertThat(XContentType.fromMediaTypeOrFormat("application/vnd.renameme+smile;compatible-with=7"),
            equalTo(XContentType.SMILE));

        assertThat(XContentType.fromMediaTypeOrFormat("application/vnd.renameme+json ;compatible-with=7"),
            equalTo(XContentType.JSON));
        assertThat(XContentType.fromMediaTypeOrFormat("application/vnd.renameme+json ;compatible-with=7;charset=utf-8"),
            equalTo(XContentType.JSON));
    }
}
