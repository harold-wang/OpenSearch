...
import org.renameme.common.lucene.ShardCoreKeyMap;
...
public class IndicesQueryCache implements QueryCache, Closeable {
    ...
    public IndicesQueryCache(Settings settings) {
        ...
        if (INDICES_QUERIES_CACHE_ALL_SEGMENTS_SETTING.get(settings)) {
            cache = new RenamemeLRUQueryCache(count, size.getBytes(), context -> true, 1f);
        } else {
            cache = new RenamemeLRUQueryCache(count, size.getBytes());
        }
    ...
    private class RenamemeLRUQueryCache extends LRUQueryCache {

        RenamemeLRUQueryCache(int maxSize, long maxRamBytesUsed, Predicate<LeafReaderContext> leavesToCache, float skipFactor) {
            super(maxSize, maxRamBytesUsed, leavesToCache, skipFactor);
        }

        RenamemeLRUQueryCache(int maxSize, long maxRamBytesUsed) {
