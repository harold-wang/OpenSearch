...
package org.renameme.cluster.routing.allocation.command;
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if ("index".equals(currentFieldName)) {
                    ...
                    throw new RenamemeParseException("[{}] command does not support field [{}]", NAME, currentFieldName);
                }
            } else {
                throw new RenamemeParseException("[{}] command does not support complex json tokens [{}]", NAME, token);
            }
        }
        if (index == null) {
            throw new RenamemeParseException("[{}] command missing the index parameter", NAME);
        }
        if (shardId == -1) {
            throw new RenamemeParseException("[{}] command missing the shard parameter", NAME);
        }
        if (nodeId == null) {
            throw new RenamemeParseException("[{}] command missing the node parameter", NAME);
        }
