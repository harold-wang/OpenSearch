...
import org.renameme.index.snapshots.blobstore.BlobStoreIndexShardSnapshot.FileInfo;
            ...
            while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
                ...
                if (token == XContentParser.Token.START_ARRAY) {
                    if (ParseFields.FILES.match(currentFieldName, parser.getDeprecationHandler()) == false) {
                        throw new RenamemeParseException("unknown array [{}]", currentFieldName);
                    }
                    ...
                    if (ParseFields.SNAPSHOTS.match(currentFieldName, parser.getDeprecationHandler()) == false) {
                        throw new RenamemeParseException("unknown object [{}]", currentFieldName);
                    }
                    while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
                        XContentParserUtils.ensureExpectedToken(XContentParser.Token.FIELD_NAME, token, parser);
