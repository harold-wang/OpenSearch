...
import org.renameme.index.reindex.BulkByScrollTask.Status;
...
public class BulkByScrollResponse extends ActionResponse implements ToXContentFragment {
    ...
    private static Object parseFailure(XContentParser parser) throws IOException {
       ...
       String nodeId = null;
       RenamemeException bulkExc = null;
       RenamemeException searchExc = null;
           ...
           if (token == Token.START_ARRAY) {
               ...
               switch (name) {
                   case SearchFailure.REASON_FIELD:
                       searchExc = RenamemeException.fromXContent(parser);
                       break;
                   case Failure.CAUSE_FIELD:
                       bulkExc = RenamemeException.fromXContent(parser);
                       break;
                   default:
                       parser.skipChildren();
               }
       ...
       if (bulkExc != null) {
           ...
           throw new RenamemeParseException("failed to parse failures array. At least one of {reason,cause} must be present");
       }
    }
