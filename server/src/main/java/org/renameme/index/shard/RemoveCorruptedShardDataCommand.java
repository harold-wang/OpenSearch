...
import org.renameme.cluster.routing.allocation.command.AllocateStalePrimaryAllocationCommand;
...
import org.renameme.index.translog.TruncateTranslogAction;
...
public class RemoveCorruptedShardDataCommand extends RenamemeNodeCommand {
    ...
    throws IOException {
        ...
        if (options.has(folderOption)) {
            ...
            if (Files.exists(indexPath) == false || Files.isDirectory(indexPath) == false) {
                throw new RenamemeException("index directory [" + indexPath + "], must exist and be a directory");
            }
            ...
            if (Files.isDirectory(path) && shardIdFileName.chars().allMatch(Character::isDigit) // SHARD-ID path element check
                ...
                throw new RenamemeException("Unable to resolve shard id. Wrong folder structure at [ " + path.toString()
        ...
        if (indexMetadata == null) {
            throw new RenamemeException("Unable to find index in cluster state");
        }
        ...
        throw new RenamemeException("Unable to resolve shard path for index [" + indexMetadata.getIndex().getName() +
    ...
    private static void confirm(String msg, Terminal terminal) {
        ...
        if (text.equalsIgnoreCase("y") == false) {
            throw new RenamemeException("aborted by user");
        }
    }
    ...
    public void processNodePaths(Terminal terminal, Path[] dataPaths, int nodeLockId, OptionSet options, Environment environment)
        ...
        findAndProcessShardPath(options, environment, dataPaths, nodeLockId, clusterState, shardPath -> {
            ...
            if (Files.exists(translogPath) == false || Files.isDirectory(translogPath) == false) {
                throw new RenamemeException("translog directory [" + translogPath + "], must exist and be a directory");
            }
            ...
            try (Directory indexDir = indexDirectory) {
                ...
                try (Lock writeIndexLock = indexDir.obtainLock(IndexWriter.WRITE_LOCK_NAME)) {
                    ...
                    if (indexStatus == CleanStatus.CLEAN && translogStatus == CleanStatus.CLEAN) {
                        throw new RenamemeException("Shard does not seem to be corrupted at " + shardPath.getDataPath()
                    ...
                    if (indexStatus == CleanStatus.UNRECOVERABLE) {
                        ...
                        throw new RenamemeException("Index is unrecoverable");
                    }
                    ...
                    terminal.println(msg);
                    throw new RenamemeException(msg);
                }
    ...
    private Directory getDirectory(Path indexPath) {
        ...
        try {
            ...
            throw new RenamemeException("ERROR: could not open directory \"" + indexPath + "\"; exiting");
        }
        return directory;
    }
    ...
    private void newAllocationId(ShardPath shardPath, Terminal terminal) throws IOException {
        ...
        if (shardStateMetadata == null) {
            throw new RenamemeException("No shard state meta data at " + shardStatePath);
        }
    ...
    private void printRerouteCommand(ShardPath shardPath, Terminal terminal, boolean allocateStale)
        ...
        if (nodeMetadata == null) {
            throw new RenamemeException("No node meta data at " + nodePath);
        }
    ...
    private Path getNodePath(ShardPath shardPath) {
        ...
        if (Files.exists(nodePath) == false ||
            ...
            throw new RenamemeException("Unable to resolve node path for " + shardPath);
        }
        return nodePath;
    }
