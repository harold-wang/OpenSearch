...
public class VectorGeoShapeQueryProcessor {
    ...
    private static class LuceneGeometryCollector implements GeometryVisitor<Void, RuntimeException> {
        ...
        @Override
        public Void visit(org.renameme.geometry.Line line) {
            if (line.isEmpty() == false) {
                List<org.renameme.geometry.Line> collector = new ArrayList<>();
        ...
        public Void visit(MultiPolygon multiPolygon) {
            if (multiPolygon.isEmpty() == false) {
                List<org.renameme.geometry.Polygon> collector = new ArrayList<>();
        ...
        @Override
        public Void visit(org.renameme.geometry.Polygon polygon) {
        ...
        private void collectLines(List<org.renameme.geometry.Line> geometryLines) {
            for (Line line: geometryLines) {
                geometries.add(GeoShapeUtils.toLuceneLine(line));
            }
        }

        private void collectPolygons(List<org.renameme.geometry.Polygon> geometryPolygons) {
