...
import org.renameme.cluster.NotMasterException;
import org.renameme.cluster.block.ClusterBlocks;
import org.renameme.cluster.coordination.FailedToCommitClusterStateException;
...
import org.renameme.discovery.zen.PublishClusterStateAction.IncomingClusterStateListener;
import org.renameme.tasks.Task;
import org.renameme.threadpool.ThreadPool;
import org.renameme.transport.EmptyTransportResponseHandler;
        ...
        try {
            ...
            synchronized (stateMutex) {
                pendingStatesQueue.failAllStatesAndClear(
                    new RenamemeException("failed to publish cluster state"));
    ...
    private void handleMasterGone(final DiscoveryNode masterNode, final Throwable cause, final String reason) {
        ...
        synchronized (stateMutex) {
            if (localNodeMaster() == false && masterNode.equals(committedState.get().nodes().getMasterNode())) {
                ...
                pendingStatesQueue.failAllStatesAndClear(new RenamemeException("master left [{}]", reason));
