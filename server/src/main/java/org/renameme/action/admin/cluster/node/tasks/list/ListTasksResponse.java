...
package org.renameme.action.admin.cluster.node.tasks.list;
    ...
    protected static <T> ConstructingObjectParser<T, Void> setupParser(String name,
        ...
        ConstructingObjectParser<T, Void> parser = new ConstructingObjectParser<>(name, true,
            constructingObjects -> {
                ...
                @SuppressWarnings("unchecked")
                List<RenamemeException> nodeFailures = (List<RenamemeException>) constructingObjects[i];
        ...
        parser.declareObjectArray(optionalConstructorArg(),
            (p, c) -> RenamemeException.fromXContent(p), new ParseField(NODE_FAILURES));
        return parser;
    }
