...
package org.renameme.action.admin.cluster.stats;
...
import org.renameme.plugins.PluginInfo;
import org.renameme.transport.TransportInfo;
...
public class ClusterStatsNodes implements ToXContentFragment {
    ...
    public static class OsStats implements ToXContentFragment {
        ...
        final org.renameme.monitor.os.OsStats.Mem mem;
        ...
        private OsStats(List<NodeInfo> nodeInfos, List<NodeStats> nodeStatsList) {
            ...
            this.mem = new org.renameme.monitor.os.OsStats.Mem(totalMemory, freeMemory);
        }
        ...
        public org.renameme.monitor.os.OsStats.Mem getMem() {
            return mem;
        }
    ...
    public static class JvmStats implements ToXContentFragment {
        ...
        private JvmStats(List<NodeInfo> nodeInfos, List<NodeStats> nodeStatsList) {
            ...
            for (NodeStats nodeStats : nodeStatsList) {
                org.renameme.monitor.jvm.JvmStats js = nodeStats.getJvm();
                if (js == null) {
                    continue;
                }
            ...
            for (NodeStats nodeStat : nodeStats) {
                if (nodeStat.getIngestStats() != null) {
                    for (Map.Entry<String,
                            List<org.renameme.ingest.IngestStats.ProcessorStat>> processorStats : nodeStat.getIngestStats()
                            .getProcessorStats().entrySet()) {
                        pipelineIds.add(processorStats.getKey());
                        for (org.renameme.ingest.IngestStats.ProcessorStat stat : processorStats.getValue()) {
                            stats.compute(stat.getType(), (k, v) -> {
                                org.renameme.ingest.IngestStats.Stats nodeIngestStats = stat.getStats();
                                if (v == null) {
                                    return new long[] {
