 ...
 * under the License.
 */
package org.renameme.action.admin.indices.analyze;
    ...
    private static List<AnalyzeAction.AnalyzeToken> simpleAnalyze(AnalyzeAction.Request request,
        ...
        for (String text : request.text()) {
            try (TokenStream stream = analyzer.tokenStream("", text)) {
                ...
                throw new RenamemeException("failed to analyze", e);
            }
        }
        return tokens;
    }
    ...
    private static String writeCharStream(Reader input) {
        ...
        do {
            try {
                ...
                throw new RenamemeException("failed to analyze (charFiltering)", e);
            }
            if (len > 0) {
                sb.append(buf, 0, len);
            }
