...
package org.renameme.action.admin.cluster.node.tasks.get;
...
import static org.renameme.action.admin.cluster.node.tasks.list.TransportListTasksAction.waitForCompletionTimeout;
...
public class TransportGetTaskAction extends HandledTransportAction<GetTaskRequest, GetTaskResponse> {
    ...
    void onGetFinishedTaskFromIndex(GetResponse response, ActionListener<GetTaskResponse> listener) throws IOException {
        ...
        if (response.isSourceEmpty()) {
            listener.onFailure(new RenamemeException("Stored task status for [{}] didn't contain any source!", response.getId()));
            return;
        }
