...
public class SearchPhaseExecutionException extends RenamemeException {
    ...
    public Throwable getCause() {
        ...
        if (cause == null) {
            ...
            for (RenamemeException rootCause : guessRootCauses()) {
                return rootCause;
            }
        }
        return cause;
    ...
    public RenamemeException[] guessRootCauses() {
        ShardOperationFailedException[] failures = ExceptionsHelper.groupBy(shardFailures);
        List<RenamemeException> rootCauses = new ArrayList<>(failures.length);
        for (ShardOperationFailedException failure : failures) {
            RenamemeException[] guessRootCauses = RenamemeException.guessRootCauses(failure.getCause());
            rootCauses.addAll(Arrays.asList(guessRootCauses));
        }
        return rootCauses.toArray(new RenamemeException[0]);
    }
