...
public class MultiSearchResponse extends ActionResponse implements Iterable<MultiSearchResponse.Item>, ToXContentObject {
    ...
    public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        for (Item item : items) {
            builder.startObject();
            if (item.isFailure()) {
                RenamemeException.generateFailureXContent(builder, params, item.getFailure(), true);
    ...
    private static MultiSearchResponse.Item itemFromXContent(XContentParser parser) throws IOException {
        ...
        // 2) Each response json object gets a status field injected that RenamemeException.failureFromXContent(...) does not parse,
            ...
            switch (token) {
                case FIELD_NAME:
                    ...
                    if ("error".equals(fieldName)) {
                        item = new Item(null, RenamemeException.failureFromXContent(parser));
