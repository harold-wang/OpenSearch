                ...
                if (token == Token.FIELD_NAME) {
                    ...
                    if (INDEX.match(currentFieldName, parser.getDeprecationHandler())) {
                        ...
                        if (parser.isBooleanValue()) {
                            ...
                            throw new RenamemeParseException("illegal type for _source: [{}]", token);
                        }
                    } else {
                        throw new RenamemeParseException("failed to parse multi get request. unknown field [{}]", currentFieldName);
                    }
                } else if (token == Token.START_ARRAY) {
                        ...
                        while ((token = parser.nextToken()) != Token.END_OBJECT) {
                            if (token == Token.FIELD_NAME) {
                                ...
                                if ("includes".equals(currentFieldName) || "include".equals(currentFieldName)) {
                                    ...
                                    throw new RenamemeParseException("source definition may not contain [{}]", parser.text());
                                }
                            } else if (token == Token.START_ARRAY) {
                                while ((token = parser.nextToken()) != Token.END_ARRAY) {
                                ...
                                throw new RenamemeParseException("unexpected token while parsing source settings");
                            }
                        }
