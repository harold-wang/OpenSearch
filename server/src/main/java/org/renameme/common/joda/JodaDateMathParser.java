...
public class JodaDateMathParser implements DateMathParser {
    ...
    public Instant parse(String text, LongSupplier now, boolean roundUp, ZoneId tz) {
        ...
        if (text.startsWith("now")) {
            try {
                ...
                throw new RenamemeParseException("could not read the current timestamp", e);
            }
    ...
    private long parseMath(String mathString, long time, boolean roundUp, DateTimeZone timeZone) throws RenamemeParseException {
        if (timeZone == null) {
        ...
        for (int i = 0; i < mathString.length(); ) {
            ...
            if (c == '/') {
                ...
                if (c == '+') {
                    ...
                    throw new RenamemeParseException("operator not supported for date math [{}]", mathString);
                }
            }

            if (i >= mathString.length()) {
                throw new RenamemeParseException("truncated date math [{}]", mathString);
            }
            ...
            if (round) {
                if (num != 1) {
                    throw new RenamemeParseException("rounding `/` can only be used on single unit types [{}]", mathString);
                }
            }
            ...
            switch (unit) {
                ...
                case 's':
                    ...
                    break;
                default:
                    throw new RenamemeParseException("unit [{}] not supported for date math [{}]", unit, mathString);
            }
    ...
    private long parseDateTime(String value, DateTimeZone timeZone, boolean roundUpIfNoTime) {
        ...
        try {
            ...
            throw new RenamemeParseException("failed to parse date field [{}] with format [{}]", e, value,
