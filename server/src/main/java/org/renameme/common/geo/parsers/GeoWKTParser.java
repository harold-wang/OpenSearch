 ...
 * under the License.
 */
package org.renameme.common.geo.parsers;
...
import org.renameme.common.geo.builders.EnvelopeBuilder;
import org.renameme.common.geo.builders.GeometryCollectionBuilder;
import org.renameme.common.geo.builders.LineStringBuilder;
import org.renameme.common.geo.builders.MultiLineStringBuilder;
import org.renameme.common.geo.builders.MultiPointBuilder;
import org.renameme.common.geo.builders.MultiPolygonBuilder;
import org.renameme.common.geo.builders.PointBuilder;
import org.renameme.common.geo.builders.PolygonBuilder;
...
import org.renameme.index.mapper.AbstractShapeGeometryFieldMapper;
...
public class GeoWKTParser {
    ...
    public static ShapeBuilder parse(XContentParser parser, final AbstractShapeGeometryFieldMapper shapeMapper)
            throws IOException, RenamemeParseException {
    ...
    private static ShapeBuilder parseGeometry(StreamTokenizer stream, GeoShapeType shapeType, final boolean ignoreZValue,
        ...
        if (shapeType != null && shapeType != GeoShapeType.GEOMETRYCOLLECTION) {
            if (type.wktName().equals(shapeType.wktName()) == false) {
                throw new RenamemeParseException("Expected geometry type [{}] but found [{}]", shapeType, type);
            }
        }
        switch (type) {
            case POINT:
    ...
    private static EnvelopeBuilder parseBBox(StreamTokenizer stream) throws IOException, RenamemeParseException {
    ...
    private static List<Coordinate> parseCoordinateList(StreamTokenizer stream, final boolean ignoreZValue, final boolean coerce)
        ...
        if (isOpenParen && nextCloser(stream).equals(RPAREN) == false) {
            throw new RenamemeParseException("expected: [{}]" + RPAREN + " but found: [{}]" + tokenString(stream), stream.lineno());
        }
        ...
        if (coordinates.size() >= coordinatesNeeded) {
            if (!coordinates.get(0).equals(coordinates.get(coordinates.size() - 1))) {
                if (coerce) {
                    ...
                    throw new RenamemeParseException("invalid LinearRing found (coordinates are not closed)");
                }
            }
        }
        if (coordinates.size() < 4) {
            throw new RenamemeParseException("invalid number of points in LinearRing (found [{}] - must be >= 4)",
                coordinates.size());
        }
    ...
    private static String nextWord(StreamTokenizer stream) throws RenamemeParseException, IOException {
        ...
        throw new RenamemeParseException("expected word but found: " + tokenString(stream), stream.lineno());
    }

    private static double nextNumber(StreamTokenizer stream) throws IOException, RenamemeParseException {
        if (stream.nextToken() == StreamTokenizer.TT_WORD) {
            if (stream.sval.equalsIgnoreCase(NAN)) {
                ...
                try {
                    ...
                    throw new RenamemeParseException("invalid number found: " + stream.sval, stream.lineno());
                }
            }
        }
        throw new RenamemeParseException("expected number but found: " + tokenString(stream), stream.lineno());
    }
    ...
    private static String nextEmptyOrOpen(StreamTokenizer stream) throws IOException, RenamemeParseException {
        final String next = nextWord(stream);
        if (next.equals(EMPTY) || next.equals(LPAREN)) {
            return next;
        }
        throw new RenamemeParseException("expected " + EMPTY + " or " + LPAREN
            + " but found: " + tokenString(stream), stream.lineno());
    }

    private static String nextCloser(StreamTokenizer stream) throws IOException, RenamemeParseException {
    ...
    private static String nextComma(StreamTokenizer stream) throws IOException, RenamemeParseException {
        if (nextWord(stream).equals(COMMA)) {
            return COMMA;
        }
        throw new RenamemeParseException("expected " + COMMA + " but found: " + tokenString(stream), stream.lineno());
    }

    private static String nextCloserOrComma(StreamTokenizer stream) throws IOException, RenamemeParseException {
        String token = nextWord(stream);
        if (token.equals(COMMA) || token.equals(RPAREN)) {
            return token;
        }
        throw new RenamemeParseException("expected " + COMMA + " or " + RPAREN
            + " but found: " + tokenString(stream), stream.lineno());
    }

    /** next word in the stream */
    private static void checkEOF(StreamTokenizer stream) throws RenamemeParseException, IOException {
        if (stream.nextToken() != StreamTokenizer.TT_EOF) {
            throw new RenamemeParseException("expected end of WKT string but found additional text: "
