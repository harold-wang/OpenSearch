...
import org.renameme.common.geo.GeoUtils.EffectivePoint;
...
import static org.renameme.index.mapper.AbstractPointGeometryFieldMapper.Names.IGNORE_Z_VALUE;
...
public class GeoPoint implements ToXContentFragment {
    ...
    public GeoPoint resetFromCoordinates(String value, final boolean ignoreZValue) {
        ...
        if (vals.length > 3) {
            throw new RenamemeParseException("failed to parse [{}], expected 2 or 3 coordinates "
    ...
    private GeoPoint resetFromWKT(String value, boolean ignoreZValue) {
        ...
        try {
            ...
            throw new RenamemeParseException("Invalid WKT format", e);
        }
        if (geometry.type() != ShapeType.POINT) {
            throw new RenamemeParseException("[geo_point] supports only POINT among WKT primitives, " +
    ...
    public GeoPoint resetFromGeoHash(String geohash) {
        ...
        try {
            ...
            throw new RenamemeParseException(ex.getMessage(), ex);
        }
    ...
    public static double assertZValue(final boolean ignoreZValue, double zValue) {
        if (ignoreZValue == false) {
            throw new RenamemeParseException("Exception parsing coordinates: found Z value [{}] but [{}] "
