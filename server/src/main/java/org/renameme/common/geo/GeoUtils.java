...
import org.renameme.common.xcontent.XContentParser.Token;
import org.renameme.common.xcontent.XContentSubParser;
import org.renameme.common.xcontent.support.MapXContentParser;
import org.renameme.common.xcontent.support.XContentMapValues;
import org.renameme.index.fielddata.FieldData;
import org.renameme.index.fielddata.GeoPointValues;
import org.renameme.index.fielddata.MultiGeoPointValues;
import org.renameme.index.fielddata.NumericDoubleValues;
import org.renameme.index.fielddata.SortedNumericDoubleValues;
import org.renameme.index.fielddata.SortingNumericDoubleValues;
...
public class GeoUtils {
     ...
     */
    public static GeoPoint parseGeoPoint(XContentParser parser) throws IOException, RenamemeParseException {
        return parseGeoPoint(parser, new GeoPoint());
    }


    public static GeoPoint parseGeoPoint(XContentParser parser, GeoPoint point) throws IOException, RenamemeParseException {
     ...
     */
    public static GeoPoint parseGeoPoint(Object value, final boolean ignoreZValue) throws RenamemeParseException {
     ...
     * Array: two or more elements, the first element is longitude, the second is latitude, the rest is ignored if ignoreZValue is true
     */
    public static GeoPoint parseGeoPoint(Object value, GeoPoint point, final boolean ignoreZValue) throws RenamemeParseException {
        try (XContentParser parser = new MapXContentParser(NamedXContentRegistry.EMPTY, LoggingDeprecationHandler.INSTANCE,
            ...
            throw new RenamemeParseException("error parsing geopoint", ex);
        }
    }
    ...
    public static GeoPoint parseGeoPoint(XContentParser parser, GeoPoint point, final boolean ignoreZValue, EffectivePoint effectivePoint)
        ...
        if(parser.currentToken() == Token.START_OBJECT) {
                        ...
                        if (LATITUDE.equals(field)) {
                            ...
                            switch (subParser.currentToken()) {
                                ...
                                case VALUE_STRING:
                                    ...
                                    break;
                                default:
                                    throw new RenamemeParseException("latitude must be a number");
                            }
                            ...
                            switch (subParser.currentToken()) {
                                ...
                                case VALUE_STRING:
                                    ...
                                    break;
                                default:
                                    throw new RenamemeParseException("longitude must be a number");
                            }
                        } else if (GEOHASH.equals(field)) {
                            if (subParser.nextToken() == Token.VALUE_STRING) {
                                geohash = subParser.text();
                            } else {
                                throw new RenamemeParseException("geohash must be a string");
                            }
                        } else {
                            throw new RenamemeParseException("field must be either [{}], [{}] or [{}]", LATITUDE, LONGITUDE, GEOHASH);
                        }
                    } else {
                        throw new RenamemeParseException("token [{}] not allowed", subParser.currentToken());
                    }
                }
            }
            if (geohash != null) {
                if(!Double.isNaN(lat) || !Double.isNaN(lon)) {
                    throw new RenamemeParseException("field must be either lat/lon or geohash");
                } else {
                    return point.parseGeoHash(geohash, effectivePoint);
                }
            } else if (numberFormatException != null) {
                throw new RenamemeParseException("[{}] and [{}] must be valid double values", numberFormatException, LATITUDE,
                    LONGITUDE);
            } else if (Double.isNaN(lat)) {
                throw new RenamemeParseException("field [{}] missing", LATITUDE);
            } else if (Double.isNaN(lon)) {
                throw new RenamemeParseException("field [{}] missing", LONGITUDE);
            } else {
                ...
                while (subParser.nextToken() != Token.END_ARRAY) {
                    if (subParser.currentToken() == Token.VALUE_NUMBER) {
                        ...
                        if (element == 1) {
                            ...
                            throw new RenamemeParseException("[geo_point] field type does not accept > 3 dimensions");
                        }
                    } else {
                        throw new RenamemeParseException("numeric value expected");
                    }
                }
            }
            return point.reset(lat, lon);
            ...
            throw new RenamemeParseException("geo_point expected");
        }
    }
     ...
     */
    public static int parsePrecision(XContentParser parser) throws IOException, RenamemeParseException {
