 ...
 * under the License.
 */
package org.renameme.common.lucene.index;
 ...
 */
public final class RenamemeDirectoryReader extends FilterDirectoryReader {

    private final ShardId shardId;
    private final FilterDirectoryReader.SubReaderWrapper wrapper;

    private RenamemeDirectoryReader(DirectoryReader in, FilterDirectoryReader.SubReaderWrapper wrapper,
    ...
    protected DirectoryReader doWrapDirectoryReader(DirectoryReader in) throws IOException {
        return new RenamemeDirectoryReader(in, wrapper, shardId);
    }
     ...
     */
    public static RenamemeDirectoryReader wrap(DirectoryReader reader, ShardId shardId) throws IOException {
        return new RenamemeDirectoryReader(reader, new SubReaderWrapper(shardId), shardId);
    }
    ...
    private static final class SubReaderWrapper extends FilterDirectoryReader.SubReaderWrapper {
        ...
        public LeafReader wrap(LeafReader reader) {
            return new RenamemeLeafReader(reader, shardId);
        }
    }
    ...
    public static void addReaderCloseListener(DirectoryReader reader, IndexReader.ClosedListener listener) {
        ...
        if (cacheHelper == null) {
            throw new IllegalArgumentException("Reader " + renamemeDirectoryReader + " does not support caching");
        }
