...
public class OverrideNodeVersionCommand extends RenamemeNodeCommand {
    ...
    protected void processNodePaths(Terminal terminal, Path[] dataPaths, int nodeLockId, OptionSet options, Environment env)
        ...
        if (nodeMetadata == null) {
            throw new RenamemeException(NO_METADATA_MESSAGE);
        }

        try {
            nodeMetadata.upgradeToCurrentVersion();
            throw new RenamemeException("found [" + nodeMetadata + "] which is compatible with current version [" + Version.CURRENT
