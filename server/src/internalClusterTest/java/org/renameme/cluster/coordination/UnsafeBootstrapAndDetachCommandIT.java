...
import static org.renameme.gateway.DanglingIndicesState.AUTO_IMPORT_DANGLING_INDICES_SETTING;
import static org.renameme.index.query.QueryBuilders.matchAllQuery;
import static org.renameme.indices.recovery.RecoverySettings.INDICES_RECOVERY_MAX_BYTES_PER_SEC_SETTING;
...
public class UnsafeBootstrapAndDetachCommandIT extends ESIntegTestCase {
    ...
    private MockTerminal executeCommand(RenamemeNodeCommand command, Environment environment, int nodeOrdinal, boolean abort)
            throws Exception {
        ...
        try {
            ...
            assertThat(terminal.getOutput(), containsString(RenamemeNodeCommand.STOP_WARNING_MSG));
        }
    ...
    private void expectThrows(ThrowingRunnable runnable, String message) {
        RenamemeException ex = expectThrows(RenamemeException.class, runnable);
    ...
    public void testBootstrapNoDataFolder() {
        ...
        expectThrows(() -> unsafeBootstrap(environment), RenamemeNodeCommand.NO_NODE_FOLDER_FOUND_MSG);
    }

    public void testDetachNoDataFolder() {
        final Environment environment = TestEnvironment.newEnvironment(internalCluster().getDefaultSettings());
        expectThrows(() -> detachCluster(environment), RenamemeNodeCommand.NO_NODE_FOLDER_FOUND_MSG);
    }
    ...
    public void testBootstrapNodeLocked() throws IOException {
        ...
        try (NodeEnvironment ignored = new NodeEnvironment(envSettings, environment)) {
            expectThrows(() -> unsafeBootstrap(environment), RenamemeNodeCommand.FAILED_TO_OBTAIN_NODE_LOCK_MSG);
        }
    }
    ...
    public void testDetachNodeLocked() throws IOException {
        ...
        Environment environment = TestEnvironment.newEnvironment(envSettings);
        try (NodeEnvironment ignored = new NodeEnvironment(envSettings, environment)) {
            expectThrows(() -> detachCluster(environment), RenamemeNodeCommand.FAILED_TO_OBTAIN_NODE_LOCK_MSG);
        }
    }
    ...
    public void testBootstrapNoClusterState() throws IOException {
        ...
        expectThrows(() -> unsafeBootstrap(environment), RenamemeNodeCommand.NO_NODE_METADATA_FOUND_MSG);
    }
    ...
    public void testDetachNoClusterState() throws IOException {
        ...
        expectThrows(() -> detachCluster(environment), RenamemeNodeCommand.NO_NODE_METADATA_FOUND_MSG);
    }
    ...
    public void testBootstrapAbortedByUser() throws IOException {
        ...
        Environment environment = TestEnvironment.newEnvironment(
            Settings.builder().put(internalCluster().getDefaultSettings()).put(dataPathSettings).build());
        expectThrows(() -> unsafeBootstrap(environment, true), RenamemeNodeCommand.ABORTED_BY_USER_MSG);
    }
    ...
    public void testDetachAbortedByUser() throws IOException {
        ...
        Environment environment = TestEnvironment.newEnvironment(
            Settings.builder().put(internalCluster().getDefaultSettings()).put(dataPathSettings).build());
        expectThrows(() -> detachCluster(environment, true), RenamemeNodeCommand.ABORTED_BY_USER_MSG);
    }
    ...
    public void test3MasterNodes2Failed() throws Exception {
        ...
        Metadata metadata = RenamemeNodeCommand.createPersistedClusterStateService(Settings.EMPTY, nodeEnvironment.nodeDataPaths())
