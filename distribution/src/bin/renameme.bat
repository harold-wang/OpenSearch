...
CALL "%~dp0renameme-env.bat" || exit /b 1
IF ERRORLEVEL 1 (
...
IF "%checkpassword%"=="Y" (
  CALL "%~dp0renameme-keystore.bat" has-passwd --silent
  IF !ERRORLEVEL! EQU 0 (
    SET /P KEYSTORE_PASSWORD=Renameme keystore password:
    IF !ERRORLEVEL! NEQ 0 (
...
ECHO.!KEYSTORE_PASSWORD!| %JAVA% %ES_JAVA_OPTS% -Drenameme ^
  -Des.path.home="%ES_HOME%" -Des.path.conf="%ES_PATH_CONF%" ^
  -Des.distribution.flavor="%ES_DISTRIBUTION_FLAVOR%" ^
  -Des.distribution.type="%ES_DISTRIBUTION_TYPE%" ^
  -Des.bundled_jdk="%ES_BUNDLED_JDK%" ^
  -cp "%ES_CLASSPATH%" "org.renameme.bootstrap.Renameme" !newparams!
