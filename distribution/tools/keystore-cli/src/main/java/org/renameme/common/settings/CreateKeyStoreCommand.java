...
class CreateKeyStoreCommand extends KeyStoreAwareCommand {
    ...
    CreateKeyStoreCommand() {
        super("Creates a new renameme keystore");
    ...
    protected void execute(Terminal terminal, OptionSet options, Environment env) throws Exception {
        try (SecureString password = options.has(passwordOption) ? readPassword(terminal, true) : new SecureString(new char[0])) {
            ...
            if (Files.exists(keystoreFile)) {
                if (terminal.promptYesNo("An renameme keystore already exists. Overwrite?", false) == false) {
            ...
            terminal.println("Created renameme keystore in " + KeyStoreWrapper.keystorePath(env.configFile()));
        } catch (SecurityException e) {
            throw new UserException(ExitCodes.IO_ERROR, "Error creating the renameme keystore.");
        }
    }
}
