...
public class KeyStoreWrapperTests extends ESTestCase {
    ...
    public void testFailWhenCannotConsumeSecretStream() throws Exception {
        ...
        try (IndexOutput indexOutput = directory.createOutput("renameme.keystore", IOContext.DEFAULT)) {
            CodecUtil.writeHeader(indexOutput, "renameme.keystore", 3);
    ...
    public void testBackcompatV1() throws Exception {
        ...
        Path configDir = env.configFile();
        SimpleFSDirectory directory = new SimpleFSDirectory(configDir);
        try (IndexOutput output = directory.createOutput("renameme.keystore", IOContext.DEFAULT)) {
            CodecUtil.writeHeader(output, "renameme.keystore", 1);
    ...
    public void testBackcompatV2() throws Exception {
        ...
        try (IndexOutput output = directory.createOutput("renameme.keystore", IOContext.DEFAULT)) {
            ...
            CodecUtil.writeHeader(output, "renameme.keystore", 2);
            output.writeByte((byte) 0); // hasPassword = false
            output.writeString("PKCS12");
    ...
    public void testLegacyV3() throws GeneralSecurityException, IOException {
        ...
        final Path keystore = configDir.resolve("renameme.keystore");
        try (
            InputStream is = KeyStoreWrapperTests.class.getResourceAsStream("/format-v3-renameme.keystore");
