...
import org.renameme.test.PosixPermissionsResetter;
...
public class InstallPluginCommandTests extends ESTestCase {
    ...
    static Tuple<Path, Environment> createEnv(FileSystem fs, Function<String, Path> temp) throws IOException {
        ...
        Files.createFile(home.resolve("bin").resolve("renameme"));
        Files.createDirectories(home.resolve("config"));
        Files.createFile(home.resolve("config").resolve("renameme.yml"));
    ...
    public void testInstallFailsIfPreviouslyRemovedPluginFailed() throws Exception {
        ...
        final String expected = String.format(
            Locale.ROOT,
            "found file [%s] from a failed attempt to remove the plugin [failed]; execute [renameme-plugin remove failed]",
            removing
        );
    ...
    public void testBinConflict() throws Exception {
        ...
        Files.createDirectory(binDir);
        Files.createFile(binDir.resolve("somescript"));
        String pluginZip = createPluginUrl("renameme", pluginDir);
        FileAlreadyExistsException e = expectThrows(FileAlreadyExistsException.class, () -> installPlugin(pluginZip, env.v1()));
        assertTrue(e.getMessage(), e.getMessage().contains(env.v2().binFile().resolve("renameme").toString()));
        assertInstallCleaned(env.v2());
    }
    ...
    public void testContainsIntermediateDirectory() throws Exception {
        ...
        String pluginZip = writeZip(pluginDir, "renameme").toUri().toURL().toString();
        UserException e = expectThrows(UserException.class, () -> installPlugin(pluginZip, env.v1()));
    ...
    public void testOfficialPlugin() throws Exception {
        String url = "https://artifacts.renameme.co/downloads/renameme-plugins/analysis-icu/analysis-icu-"
    ...
    public void testOfficialPluginSnapshot() throws Exception {
        String url = String.format(
            Locale.ROOT,
            "https://snapshots.renameme.co/%s-abc123/downloads/renameme-plugins/analysis-icu/analysis-icu-%s.zip",
            Version.CURRENT,
            Build.CURRENT.getQualifiedVersion()
        );
    ...
    public void testOfficialPluginStaging() throws Exception {
        String url = "https://staging.elastic.co/"
            + Version.CURRENT
            + "-abc123/downloads/renameme-plugins/analysis-icu/analysis-icu-"
            + Build.CURRENT.getQualifiedVersion()
            + ".zip";
    ...
    public void testOfficialPlatformPluginSnapshot() throws Exception {
        String url = String.format(
            Locale.ROOT,
            "https://snapshots.renameme.co/%s-abc123/downloads/renameme-plugins/analysis-icu/analysis-icu-%s-%s.zip",
            Version.CURRENT,
            Platforms.PLATFORM_NAME,
            Build.CURRENT.getQualifiedVersion()
        );
    ...
    public void testPublicKeyIdMismatchToExpectedPublicKeyId() throws Exception {
        ...
        final String url = "https://artifacts.renameme.co/downloads/renameme-plugins/analysis-icu/"
            + icu
            + "-"
            + Build.CURRENT.getQualifiedVersion()
            + ".zip";
